...
void collectBlockAddresses(const llvm::Value *Root,
                           std::set<const llvm::Value *> &Seen,
                           std::set<const llvm::BlockAddress *> &Addresses) {
  if (!Root || !Seen.insert(Root).second)
    return;
  if (const auto *Address = llvm::dyn_cast<llvm::BlockAddress>(Root)) {
    ...
    if (const auto *Slot = llvm::dyn_cast<llvm::AllocaInst>(
      ...
      for (const llvm::User *User : Slot->users())
        if (const auto *Store = llvm::dyn_cast<llvm::StoreInst>(User);
          ...
          collectBlockAddresses(Store->getValueOperand(), Seen, Addresses);
  const auto *User = llvm::dyn_cast<llvm::User>(Root);
  if (!User)
    return;
  for (const llvm::Use &Operand : User->operands())
    collectBlockAddresses(Operand.get(), Seen, Addresses);
}

bool observableValueReferencesInteger(const llvm::Value *Root, uint64_t Value,
                                      std::set<const llvm::Value *> &Seen) {
  if (!Root || !Seen.insert(Root).second)
    return false;
  // A blockaddress is the relocatable terminal we are proving. Its internal
  // LLVM operands are ownership objects, not source integer-value flow.
  if (llvm::isa<llvm::BlockAddress>(Root))
    return false;
  ...
  if (const auto *Load = llvm::dyn_cast<llvm::LoadInst>(Root))
    if (const auto *Slot = llvm::dyn_cast<llvm::AllocaInst>(
      ...
      for (const llvm::User *User : Slot->users())
        if (const auto *Store = llvm::dyn_cast<llvm::StoreInst>(User);
            Store && Store->getPointerOperand()->stripPointerCasts() == Slot &&
            observableValueReferencesInteger(Store->getValueOperand(), Value,
                                             Seen))
          return true;
  const auto *User = llvm::dyn_cast<llvm::User>(Root);
  if (!User)
    return false;
  for (const llvm::Use &Operand : User->operands())
    if (observableValueReferencesInteger(Operand.get(), Value, Seen))
      return true;
  return false;
}
...
void addImportStub(BinaryImage &Image, llvm::StringRef Name, va_t Address) {
  addImport(Image, Name, Address);
  ASSERT_TRUE(Image.recordImportStub(Address, Image.Imports.size() - 1));
}
...
BinaryImage makeLLVMImage() {
  ...
  addImportStub(Image, "_getopt_long", ImportStubVA);
  return Image;
}
...
TEST(MachOChainedPointerBoundary, RecordsBindAndFineGrainedRebases) {
  ...
  // not ordinal 0's addend 99; the unsigned pointer addend contributes 254.
  EXPECT_EQ(Image.DyldBindSlots.at(DataVA).Addend, 261);
...
TEST(MachOChainedPointerBoundary,
  ...
  // The table addend is signed and the pointer addend is unsigned. Their sum
  // must be checked before a binding becomes observable.
  {
    BinaryImage Image = makeChainedImage();
...
makeChainedNumericBlob(macho_loader::ChainedFixupsInfo &Info,
  ...
  writeObject(Bytes, Info.DataOff, Header);
  dyld_chained_starts_in_image Starts{};
  const size_t StartsAbs = Info.DataOff + Header.starts_offset;
  std::memcpy(&Starts, Bytes.data() + StartsAbs, sizeof(Starts));
  const size_t SegmentAbs = StartsAbs + Starts.seg_info_offset[0];
  dyld_chained_starts_in_segment SegmentStarts{};
  std::memcpy(&SegmentStarts, Bytes.data() + SegmentAbs, sizeof(SegmentStarts));
  SegmentStarts.pointer_format = PointerFormat;
  writeObject(Bytes, SegmentAbs, SegmentStarts);
  constexpr char Strings[] = "_table_zero\0_numeric_target\0";
  std::memcpy(Bytes.data() + Info.DataOff + Header.symbols_offset, Strings,
              sizeof(Strings));
  const size_t ImportsAbs = Info.DataOff + Header.imports_offset;
  auto WritePair = [&](auto Entry) {
    Entry.lib_ordinal = 1;
    Entry.name_offset = 0;
    writeObject(Bytes, ImportsAbs, Entry);
    Entry.lib_ordinal = LibraryOrdinal;
    Entry.name_offset = sizeof("_table_zero");
    Entry.weak_import = SymbolWeak;
    writeObject(Bytes, ImportsAbs + sizeof(Entry), Entry);
  };
  switch (ImportFormat) {
  case DYLD_CHAINED_IMPORT:
    WritePair(dyld_chained_import{});
    break;
  case DYLD_CHAINED_IMPORT_ADDEND: {
    dyld_chained_import_addend Entry{};
    Entry.addend = static_cast<int32_t>(OutlineAddend);
    WritePair(Entry);
    break;
  }
  case DYLD_CHAINED_IMPORT_ADDEND64: {
    dyld_chained_import_addend64 Entry{};
    Entry.addend = static_cast<uint64_t>(OutlineAddend);
    WritePair(Entry);
    break;
  }
  default:
    ADD_FAILURE() << "unsupported owned import format " << ImportFormat;
  }
  return Bytes;
}

BinaryImage makeChainedNumericImage(uint8_t InlineAddend) {
  auto Image = makeChainedImage();
  Image.MachODylibReferences = {{"native-first-library", false}};
  dyld_chained_ptr_64_bind Bind{};
  Bind.ordinal = 1;
  Bind.addend = InlineAddend;
  Bind.bind = 1;
  writeObject(Image.Segments[1].Data, 0, Bind);
  return Image;
}

TEST(MachOChainedPointerBoundary, PreservesHighPositiveLibraryOrdinals) {
  for (uint32_t ImportFormat : {DYLD_CHAINED_IMPORT, DYLD_CHAINED_IMPORT_ADDEND,
    ...
    for (uint16_t PointerFormat :
      ...
      for (uint32_t LibraryOrdinal : Ordinals) {
        ...
        SCOPED_TRACE(LibraryOrdinal);
        auto Image = makeChainedNumericImage(0);
        Image.DynInfo.NeededLibs.assign(LibraryOrdinal, "display-other");
        Image.MachODylibReferences.assign(LibraryOrdinal,
                                          {"native-other", false});
        const std::string ExpectedModule =
            "ordinal-library-" + std::to_string(LibraryOrdinal);
        Image.DynInfo.NeededLibs[LibraryOrdinal - 1] = "display-target";
        Image.MachODylibReferences[LibraryOrdinal - 1].Name = ExpectedModule;
        macho_loader::ChainedFixupsInfo Info;
        ...
        EXPECT_EQ(Image.Imports[1].Name, "_numeric_target");
        EXPECT_EQ(Image.Imports[1].Module, "display-target");
        EXPECT_EQ(Image.Imports[1].IATAddr, DataVA);
        ...
        EXPECT_EQ(Binding.Module, ExpectedModule);
        EXPECT_EQ(Binding.Addend, 0);
        EXPECT_FALSE(Binding.WeakImport);
      }
  }
}

TEST(MachOChainedPointerBoundary,
     SpecialZeroAndOutOfRangeLibraryOrdinalsStayExternal) {
  for (uint32_t ImportFormat : {DYLD_CHAINED_IMPORT, DYLD_CHAINED_IMPORT_ADDEND,
                                DYLD_CHAINED_IMPORT_ADDEND64}) {
    const uint32_t LastEncoding =
        ImportFormat == DYLD_CHAINED_IMPORT_ADDEND64 ? 0xffff : 0xff;
    std::vector<uint32_t> Ordinals{0, 2};
    ...
    for (uint16_t PointerFormat :
      ...
      for (uint32_t LibraryOrdinal : Ordinals) {
        ...
        SCOPED_TRACE(LibraryOrdinal);
        auto Image = makeChainedNumericImage(0);
        const size_t LibraryCount = LibraryOrdinal == 2 ? 1 : LastEncoding;
        // Back special encodings with real sentinel entries to reject an
        // all-unsigned implementation, not merely an out-of-range lookup.
        Image.DynInfo.NeededLibs.assign(LibraryCount, "display-reserved");
        Image.DynInfo.NeededLibs.front() = "display-first";
        Image.MachODylibReferences.assign(LibraryCount,
                                          {"native-reserved", false});
        Image.MachODylibReferences.front().Name = "native-first";
        macho_loader::ChainedFixupsInfo Info;
        auto Bytes = makeChainedNumericBlob(Info, ImportFormat, PointerFormat,
                                            LibraryOrdinal, 0);
        macho_loader::parseChainedFixupsImports(Bytes.data(), Bytes.size(),
        ...
        EXPECT_EQ(Image.Imports[0].IATAddr, 0U);
        EXPECT_EQ(Image.Imports[1].Name, "_numeric_target");
        EXPECT_EQ(Image.Imports[1].Module, kExternModule);
        EXPECT_EQ(Image.Imports[1].IATAddr, DataVA);
        ASSERT_EQ(Image.DyldBindSlots.size(), 1U);
        EXPECT_EQ(Image.DyldBindSlots.at(DataVA).Name, "_numeric_target");
        EXPECT_TRUE(Image.DyldBindSlots.at(DataVA).Module.empty());
        EXPECT_FALSE(Image.DyldBindSlots.at(DataVA).WeakImport);
      }
  }
}
  ...
  for (uint32_t ImportFormat : {DYLD_CHAINED_IMPORT, DYLD_CHAINED_IMPORT_ADDEND,
    ...
    for (uint16_t PointerFormat :
      ...
      for (uint8_t InlineAddend : {0, 127, 128, 254, 255}) {
        SCOPED_TRACE(ImportFormat);
        SCOPED_TRACE(PointerFormat);
        SCOPED_TRACE(unsigned(InlineAddend));
        auto Image = makeChainedNumericImage(InlineAddend);
        Image.CodePtrRelocSlots.insert(DataVA);
        Image.DataPtrRelocSlots.insert(DataVA);
        macho_loader::ChainedFixupsInfo Info;
        auto Bytes =
        ...
        ASSERT_EQ(Image.ImportStorageSlots.count(DataVA), 1U);
        EXPECT_EQ(Image.ImportStorageSlots.at(DataVA).Addend, InlineAddend);
        EXPECT_EQ(Image.ImportStorageSlots.at(DataVA).Evidence,
...
TEST(MachOChainedPointerBoundary,
     AddsUnsignedInlineToSignedOutlineWithinInt64Range) {
  struct Case {
    int64_t Outline;
    uint8_t Inline;
    int64_t Expected;
  ...
  for (uint32_t ImportFormat :
    ...
    for (uint16_t PointerFormat :
      ...
      for (const auto &Test : Cases) {
        SCOPED_TRACE(ImportFormat);
        SCOPED_TRACE(PointerFormat);
        SCOPED_TRACE(Test.Outline);
        SCOPED_TRACE(unsigned(Test.Inline));
        auto Image = makeChainedNumericImage(Test.Inline);
        macho_loader::ChainedFixupsInfo Info;
        auto Bytes = makeChainedNumericBlob(Info, ImportFormat, PointerFormat,
                                            1, Test.Outline);
        macho_loader::parseChainedFixupsImports(Bytes.data(), Bytes.size(),
        ...
        ASSERT_EQ(Image.ImportStorageSlots.count(DataVA), 1U);
        EXPECT_EQ(Image.ImportStorageSlots.at(DataVA).Addend, Test.Expected);
        ASSERT_EQ(Image.Imports.size(), 2U);
        EXPECT_EQ(Image.Imports[0].IATAddr, 0U);
        EXPECT_EQ(Image.Imports[1].IATAddr, DataVA);
      }
...
TEST(MachOChainedPointerBoundary,
  ...
  for (uint16_t PointerFormat :
    ...
    for (int64_t Outline : {std::numeric_limits<int64_t>::max(),
      ...
      SCOPED_TRACE(PointerFormat);
      SCOPED_TRACE(Outline);
      auto Image = makeChainedNumericImage(255);
      Image.CodePtrRelocSlots.insert(DataVA);
      Image.DataPtrRelocSlots.insert(DataVA);
      const uint64_t Encoded = readPtr(Image.readVA(DataVA, 8), true);
      macho_loader::ChainedFixupsInfo Info;
      ...
      EXPECT_EQ(Image.DataPtrRelocSlots.count(DataVA), 1U);
      EXPECT_EQ(readPtr(Image.readVA(DataVA, 8), true), Encoded);
      ASSERT_EQ(Image.Imports.size(), 2U);
      EXPECT_EQ(Image.Imports[0].IATAddr, 0U);
      EXPECT_EQ(Image.Imports[1].IATAddr, 0U);
    }
}

std::vector<uint8_t> makeProviderBindStream(int64_t LibraryOrdinal,
                                            uint8_t Offset, bool SymbolWeak,
...
void parseProviderBindStream(BinaryImage &Image,
  ...
  if (Lazy) {
    Info.LazyBindOff = 0x20;
    Info.LazyBindSize = static_cast<uint32_t>(Stream.size());
  } else {
    Info.BindOff = 0x20;
...
TEST(MachOClassicBindBoundary, RecordsNativeProviderAndWeakness) {
  struct Case {
    int64_t Ordinal;
    const char *Module;
    bool LibraryWeak;
  ...
  for (const auto &Test : Cases)
    for (bool SymbolWeak : {false, true}) {
      ...
      auto Image = makeChainedNumericImage(0);
      Image.MachODylibReferences.assign(255, {"other", false});
      Image.MachODylibReferences[0] = {"first", false};
      Image.MachODylibReferences[1] = {"second", true};
      ...
      const auto &Binding = Image.DyldBindSlots.at(DataVA);
      EXPECT_EQ(Binding.Name, "_numeric_target");
      EXPECT_EQ(Binding.Module, Test.Module);
      EXPECT_EQ(Binding.WeakImport, Test.LibraryWeak || SymbolWeak);
      EXPECT_EQ(Binding.Addend, -3);
      EXPECT_EQ(Image.ImportStorageSlots.at(DataVA).Addend, -3);
      EXPECT_TRUE(Image.ConflictingImportStorageSlots.empty());
    }
}

TEST(MachOClassicBindBoundary, ProviderConflictsAreStickyAcrossStreams) {
  for (int64_t ConflictingOrdinal : {2, 3})
    for (bool Reverse : {false, true}) {
      SCOPED_TRACE(ConflictingOrdinal);
      SCOPED_TRACE(Reverse);
      auto Image = makeChainedNumericImage(0);
      Image.MachODylibReferences = {
          {"first", false}, {"second", false}, {"first", true}};
      const auto First = makeProviderBindStream(1, 0, false);
      const auto Other = makeProviderBindStream(ConflictingOrdinal, 0, false);
      parseProviderBindStream(Image, Reverse ? Other : First);
      parseProviderBindStream(Image, Reverse ? First : Other);
      parseProviderBindStream(Image, First);
      parseProviderBindStream(Image, Other);
      EXPECT_EQ(Image.DyldBindSlots.count(DataVA), 0U);
      EXPECT_EQ(Image.ImportStorageSlots.count(DataVA), 0U);
      EXPECT_EQ(Image.ConflictingImportStorageSlots.count(DataVA), 1U);
      parseProviderBindStream(
          Image, makeProviderBindStream(ConflictingOrdinal, 8, false));
      ASSERT_EQ(Image.DyldBindSlots.size(), 1U);
      EXPECT_EQ(Image.DyldBindSlots.at(DataVA + 8).Module,
                ConflictingOrdinal == 2 ? "second" : "first");
      EXPECT_EQ(Image.DyldBindSlots.at(DataVA + 8).WeakImport,
                ConflictingOrdinal == 3);
      EXPECT_EQ(Image.collectImportStorageSlots().Slots.count(DataVA), 0U);
    }
}

TEST(MachOClassicBindBoundary, LazyEntriesResetProviderAndAddend) {
  auto Image = makeChainedNumericImage(0);
  Image.MachODylibReferences = {{"first", false}};
  auto Stream = makeProviderBindStream(1, 0, true, true);
  // The second lazy entry deliberately omits both ordinal and addend setters.
  ...
  EXPECT_TRUE(Image.DyldBindSlots.at(DataVA).WeakImport);
  EXPECT_EQ(Image.DyldBindSlots.at(DataVA).Addend, -3);
  EXPECT_TRUE(Image.DyldBindSlots.at(DataVA + 8).Module.empty());
  EXPECT_FALSE(Image.DyldBindSlots.at(DataVA + 8).WeakImport);
  EXPECT_EQ(Image.DyldBindSlots.at(DataVA + 8).Addend, 0);
}
...
TEST(MachOChainedPointerBoundary, DuplicateNamesKeepIndependentProviderSlots) {
  auto Image = makeChainedNumericImage(0);
  Image.MachODylibReferences = {{"first", false}, {"second", true}};
  dyld_chained_ptr_64_bind First{};
  First.bind = 1;
  First.ordinal = 0;
  First.next = 2;
  writeObject(Image.Segments[1].Data, 0, First);
  dyld_chained_ptr_64_bind Second{};
  Second.bind = 1;
  Second.ordinal = 1;
  writeObject(Image.Segments[1].Data, 8, Second);
  macho_loader::ChainedFixupsInfo Info;
  auto Bytes = makeChainedBlob(Info);
  dyld_chained_import_addend64 Entry{};
  Entry.lib_ordinal = 1;
  Entry.name_offset = sizeof("_unused");
  writeObject(Bytes, Info.DataOff + 0x60, Entry);
  Entry.lib_ordinal = 2;
  ...
  ASSERT_EQ(Image.DyldBindSlots.size(), 2U);
  EXPECT_EQ(Image.DyldBindSlots.at(DataVA).Name, "___stdoutp");
  EXPECT_EQ(Image.DyldBindSlots.at(DataVA + 8).Name, "___stdoutp");
  EXPECT_EQ(Image.DyldBindSlots.at(DataVA).Module, "first");
  EXPECT_EQ(Image.DyldBindSlots.at(DataVA + 8).Module, "second");
  EXPECT_FALSE(Image.DyldBindSlots.at(DataVA).WeakImport);
  EXPECT_TRUE(Image.DyldBindSlots.at(DataVA + 8).WeakImport);
}

TEST(MachOChainedPointerBoundary, ProviderConflictsAreStickyAcrossParsers) {
  for (uint32_t Format : {DYLD_CHAINED_IMPORT, DYLD_CHAINED_IMPORT_ADDEND,
                          DYLD_CHAINED_IMPORT_ADDEND64})
    for (int64_t OtherOrdinal : {2, 3})
      for (bool ChainedFirst : {false, true}) {
        SCOPED_TRACE(Format);
        SCOPED_TRACE(OtherOrdinal);
        SCOPED_TRACE(ChainedFirst);
        auto Image = makeChainedNumericImage(0);
        Image.MachODylibReferences = {
            {"first", false}, {"second", false}, {"first", true}};
        macho_loader::ChainedFixupsInfo Info;
        auto Bytes = makeChainedNumericBlob(Info, Format,
        ...
        auto Classic = [&]() {
          parseProviderBindStream(
              Image, makeProviderBindStream(OtherOrdinal, 0, false));
        };
        if (ChainedFirst) {
          Chain();
          Classic();
...
TEST(MachOLLVMDataPointerBoundary, SymbolizesExecutableCStringPhi) {
  BinaryImage Image = makeLLVMImage();
  addImportStub(Image, "_printf", ImportStubVA + 4);
...
TEST(MachOLLVMDataPointerBoundary,
  ...
  for (Arch TargetArch : {Arch::X86, Arch::X64, Arch::AArch64})
    for (bool ViaFP : {false, true}) {
      ...
      EXPECT_TRUE(MedLLVMProvenanceTestPeer::stableOffset(Proof, Reload.Output,
                                                          nullptr));

      // The proof's source and width must match the real emitted prologue.
      llvm::LLVMContext Context;
      auto Module =
          MedLLVMEmitter().emit({Func}, Context, "parameter-home", TargetArch);
      ASSERT_NE(Module, nullptr);
      expectValidModule(*Module);
      ...
      for (const llvm::BasicBlock &Block : *Function)
        for (const llvm::Instruction &Instruction : Block)
          if (const auto *Store = llvm::dyn_cast<llvm::StoreInst>(&Instruction);
            ...
            EXPECT_TRUE(
                llvm::isa<llvm::AllocaInst>(FrameEnd->getPointerOperand()));
          }
...
TEST(MachOLLVMDataPointerBoundary,
  ...
  for (Case Variant :
       {MissingHome, LiveFramePointer, WrongOffset, NarrowLoad,
        TypedWidthMismatch, PartialWrite, UnknownWrite, AtomicWrite,
        PointerWrite, ScalarWrite, OverlappingHomes, ExactHomeLast,
        UnknownHomeLast, ExactHomeAfterUnknown, InvalidHomeIndex,
        BranchScalarWrite, BranchPartialWrite, LoopUpdate}) {
    ...
    if (Variant == UnknownWrite) {
      append(Func.Blocks.front(), NdOp::INT_ADD, temp(3), {SP, Func.Params[1]});
    ...
    EXPECT_EQ(
        MedLLVMProvenanceTestPeer::stableOffset(Proof, Reload.Output, nullptr),
  ...
  for (Arch TargetArch : {Arch::X86, Arch::X64, Arch::AArch64})
    for (Scenario Case :
         {Scenario::Equal, Scenario::SubtractZero, Scenario::NotEqualFalse,
          ...
          Scenario::PointerSource, Scenario::OtherPredecessor,
          Scenario::ExceptionalSuccessor, Scenario::AmbiguousSuccessor}) {
      SCOPED_TRACE(static_cast<int>(TargetArch));
      ...
      append(Entry, NdOp::STORE, {},
             {Dynamic,
              Case == Scenario::PointerSource ? Address : Func.Params[1]});
      if (Case == Scenario::PartialAfter || Case == Scenario::ScalarAfter ||
        ...
        append(Entry, NdOp::STORE, {}, {Exact, After});
      }
      if (Case == Scenario::UnknownAfter) {
        append(Entry, NdOp::INT_ADD, value(MedVar::Temp, 5),
               {SP, Func.Params[2]});
        append(Entry, NdOp::STORE, {}, {value(MedVar::Temp, 5), scalar(42)});
      }
      ...
      if (Case == Scenario::NarrowGuard) {
        ...
        append(Entry, NdOp::SUBBYTES, Compared, {Func.Params[0], scalar(0)});
        Bound = scalar(7, 1);
      }
      ...
      append(
          Entry, NdOp::COND_BR, {},
          {scalar(LoadOnFalse ? Exit.StartAddr : Loaded.StartAddr), Condition});
      ...
      const bool Pointer =
          Case == Scenario::PointerAfter || Case == Scenario::PointerSource;
      const MedVar Source = Pointer                         ? Address
                            : Case == Scenario::ScalarAfter ? scalar(42)
                                                            : Func.Params[1];
      EXPECT_EQ(Sources, std::vector<MedVar>{Source});
      EXPECT_EQ(
          MedLLVMProvenanceTestPeer::stableOffset(Emitter, Result, nullptr),
          !Pointer);
    }
}
...
TEST(MachOLLVMDataPointerBoundary,
  ...
  for (Arch TargetArch : {Arch::X86, Arch::X64, Arch::AArch64}) {
    ...
    auto scalar = [&](uint64_t Bits) {
      return MedVar::makeConst(Bits, Width, ConstantAddressProvenance::Scalar);
    };
    MedFunc Func;
      ...
      for (NdOp Predicate :
        ...
        for (bool ViaConcat : {false, true})
          for (Scenario Case :
            ...
            EXPECT_EQ(MedLLVMProvenanceTestPeer::stableOffset(Emitter, Reloaded,
                                                              nullptr),
...
TEST(LLVMCodePointerInvariantBoundary,
     RuntimeCallableNullSlotUsesExternalOriginalRun) {
  ...
  Image.CodePtrRelocSlots.clear();
  Image.DataPtrRelocSlots.clear();
  writeObject(Image.Segments[1].Data, SlotOffset, uint64_t{0});
  ...
  llvm::GlobalVariable *OriginalRun = Module->getNamedGlobal(
      (kNdCodePtrPrefix + llvm::utohexstr(DataVA)).str());
  ASSERT_NE(OriginalRun, nullptr);
  EXPECT_TRUE(OriginalRun->isDeclaration());
  EXPECT_FALSE(OriginalRun->hasInitializer());
  EXPECT_TRUE(OriginalRun->hasExternalLinkage());
  EXPECT_FALSE(OriginalRun->isConstant());
  EXPECT_TRUE(OriginalRun->isDSOLocal());
...
TEST(LLVMCodePointerInvariantBoundary,
     RuntimeCallableCodeFallbackUsesExternalOriginalRun) {
  constexpr uint64_t SlotOffset = 8;
  constexpr va_t SlotVA = DataVA + SlotOffset;
  BinaryImage Image =
      makeMixedPointerRecordImage(Arch::X64, BinaryFormat::COFF);
  ...
  llvm::LLVMContext Context;
  auto Module = MedLLVMEmitter().emit(
      {Caller, Callee}, Context, "runtime-callable-code-fallback", Arch::X64,
...
TEST(LLVMCodePointerInvariantBoundary, RuntimeCallableDataOverlapFailsClosed) {
  ...
  Image.CodePtrRelocSlots.clear();
  Image.DataPtrRelocSlots.clear();
  writeObject(Image.Segments[1].Data, SlotOffset, CStringVA);
  Image.DataPtrRelocSlots.insert(SlotVA);
  ASSERT_TRUE(Image.recordRuntimeCallablePointerSlot(
      SlotVA, RuntimeCallablePointerSlotKind::GuardMemcpy));
  MedFunc Caller =
  ...
  EXPECT_NE(Diagnostic.find("runtime-callable pointer slot"), std::string::npos)
      << Diagnostic;
  EXPECT_NE(Diagnostic.find("conflicts with its static pointer role"),
            std::string::npos)
      << Diagnostic;
  EXPECT_NE(Diagnostic.find("refusing ambiguous storage contract"),
            std::string::npos)
      << Diagnostic;
}

TEST(LLVMCodePointerInvariantBoundary,
     UnmarkedNullPointerTableSlotFailsClosed) {
  constexpr uint64_t NullSlotOffset = 0;
  constexpr uint64_t CodeSlotOffset = 8;
  BinaryImage Image =
      makeMixedPointerRecordImage(Arch::X64, BinaryFormat::COFF);
  Image.CodePtrRelocSlots.clear();
  Image.DataPtrRelocSlots.clear();
  writeObject(Image.Segments[1].Data, NullSlotOffset, uint64_t{0});
  writeObject(Image.Segments[1].Data, CodeSlotOffset, CodeVA);
  Image.CodePtrRelocSlots.insert(DataVA + CodeSlotOffset);
  MedFunc Caller =
      makeExactMixedPointerSlotIndirectCaller(Arch::X64, NullSlotOffset);
  MedFunc Callee = makeReturnFunction("unmarked_null_neighbor", CodeVA);

  llvm::LLVMContext Context;
  testing::internal::CaptureStderr();
  auto Module = MedLLVMEmitter().emit(
      {Caller, Callee}, Context, "unmarked-null-pointer-table-slot", Arch::X64,
      {}, &Image, BinaryFormat::COFF);
  std::string Diagnostic = testing::internal::GetCapturedStderr();
  EXPECT_EQ(Module, nullptr);
...
TEST(LLVMCodePointerInvariantBoundary,
  ...
  testing::internal::CaptureStderr();
  auto Module = MedLLVMEmitter().emit(
      {Caller}, Context, "runtime-callable-unresolved-code-fallback", Arch::X64,
      {}, &Image, BinaryFormat::COFF);
  std::string Diagnostic = testing::internal::GetCapturedStderr();
  EXPECT_EQ(Module, nullptr);
  EXPECT_NE(Diagnostic.find("relocation-proven code pointer"),
            std::string::npos)
      << Diagnostic;
...
MedFunc makeMergedExactAddressReturn(ExactAddressMergeKind Kind,
                                     Arch TargetArch, va_t FirstTarget,
                                     va_t SecondTarget) {
  const TargetRegInfo &TRI = getTargetRegInfo(TargetArch);
  const uint16_t PointerSize = static_cast<uint16_t>(TRI.PointerSize);
  ...
  for (MedBlock &Block : Func.Blocks)
    for (MedOp &Op : Block.Ops)
      if (Op.Opcode == NdOp::INDIR_CALL) {
        const MedVar Merged = Op.Inputs[0];
        MedOp WriteReturn;
        WriteReturn.Opcode = NdOp::COPY;
        WriteReturn.Addr = Op.Addr;
        WriteReturn.Output = ReturnReg;
        WriteReturn.addInput(Merged);
        Op = std::move(WriteReturn);
      }
  return Func;
}

void replaceSecondMergedAddressArm(MedFunc &Func, ExactAddressMergeKind Kind,
                                   const MedVar &Replacement) {
  if (Kind == ExactAddressMergeKind::Select) {
    ASSERT_FALSE(Func.Blocks.empty());
    ASSERT_FALSE(Func.Blocks.front().Ops.empty());
    MedOp &Select = Func.Blocks.front().Ops.front();
    ASSERT_EQ(Select.Opcode, NdOp::SELECT);
    ASSERT_GE(Select.NumInputs, 3u);
    Select.Inputs[2] = Replacement;
    return;
  }

  ASSERT_GE(Func.Blocks.size(), 3u);
  ASSERT_FALSE(Func.Blocks[2].Ops.empty());
  MedOp &Copy = Func.Blocks[2].Ops.front();
  ASSERT_EQ(Copy.Opcode, NdOp::COPY);
  ASSERT_GE(Copy.NumInputs, 1u);
  Copy.Inputs[0] = Replacement;
}

MedFunc makeInteriorCodeIdentityOwner(llvm::StringRef Name, va_t EntryVA,
                                      va_t FirstInteriorVA,
                                      va_t SecondInteriorVA) {
  MedFunc Func;
  Func.Entry = EntryVA;
  Func.Name = Name.str();
  Func.ReturnType = NdType::makeVoid();

  auto AddBlock = [&](int Id, va_t Address) {
    MedBlock Block;
    Block.Id = Id;
    Block.StartAddr = Address;
    Block.EndAddr = Address + 4;
  ...
  AddBlock(0, EntryVA);
  AddBlock(1, FirstInteriorVA);
  AddBlock(2, SecondInteriorVA);
  return Func;
}
...
TEST(LLVMCodePointerInvariantBoundary,
     PreservesDistinctSameOwnerBlockAddressesAcrossValueMerges) {
  constexpr Arch TargetArch = Arch::X64;
  constexpr va_t FirstInteriorVA = CodeVA + 0x10;
  constexpr va_t SecondInteriorVA = CodeVA + 0x20;
  for (BinaryFormat Format :
       {BinaryFormat::MachO, BinaryFormat::ELF, BinaryFormat::COFF})
    for (ExactAddressMergeKind Kind :
         {ExactAddressMergeKind::Phi, ExactAddressMergeKind::Select}) {
      ...
      SCOPED_TRACE(Kind == ExactAddressMergeKind::Phi ? "phi" : "select");
      BinaryImage Image = makeSpilledConstTableImage(TargetArch, Format);
      ASSERT_TRUE(Image.isCodeAddress(FirstInteriorVA));
      ASSERT_TRUE(Image.isCodeAddress(SecondInteriorVA));

      MedFunc Caller = makeMergedExactAddressReturn(
          Kind, TargetArch, FirstInteriorVA, SecondInteriorVA);
      MedFunc Owner = makeInteriorCodeIdentityOwner(
      ...
      llvm::Function *EmittedCaller = Module->getFunction(Caller.Name);
      llvm::Function *EmittedOwner = Module->getFunction(Owner.Name);
      ASSERT_NE(EmittedCaller, nullptr);
      ASSERT_NE(EmittedOwner, nullptr);
      const llvm::ReturnInst *ObservedReturn = nullptr;
      for (const llvm::BasicBlock &Block : *EmittedCaller)
      ...
      std::set<const llvm::Value *> Seen;
      std::set<const llvm::BlockAddress *> BlockAddresses;
      collectBlockAddresses(ObservedReturn->getReturnValue(), Seen,
                            BlockAddresses);
      ASSERT_EQ(BlockAddresses.size(), 2u);
      for (const llvm::BlockAddress *Address : BlockAddresses)
        EXPECT_EQ(Address->getFunction(), EmittedOwner);
      std::set<const llvm::Value *> FirstIntegers;
      EXPECT_FALSE(observableValueReferencesInteger(
          ObservedReturn->getReturnValue(), FirstInteriorVA, FirstIntegers));
      std::set<const llvm::Value *> SecondIntegers;
      EXPECT_FALSE(observableValueReferencesInteger(
          ObservedReturn->getReturnValue(), SecondInteriorVA, SecondIntegers));
    }
}

TEST(LLVMCodePointerInvariantBoundary,
     RejectsUnmaterializableOrDataArmAcrossBlockAddressValueMerges) {
  constexpr Arch TargetArch = Arch::X64;
  constexpr va_t FirstInteriorVA = CodeVA + 0x10;
  constexpr va_t SecondInteriorVA = CodeVA + 0x20;
  const uint16_t PointerSize =
  ...
  struct Case {
    const char *Name;
    MedVar Replacement;
    const char *PhiDiagnostic;
    const char *SelectDiagnostic;
  };
  const Case Cases[] = {
      {"unlifted-code",
       MedVar::makeConst(ExecutableLayoutOnlyVA, PointerSize,
                         ConstantAddressProvenance::CodeAddress),
       "has no lifted code identity", "has no unique lifted code identity"},
      {"data-address",
       MedVar::makeConst(SpilledConstTableVA, PointerSize,
                         ConstantAddressProvenance::DataAddress),
       "has no unique lifted code identity",
       "has no unique lifted code identity"},
  };
  ...
  for (BinaryFormat Format :
    ...
    for (ExactAddressMergeKind Kind :
      ...
      for (const Case &Current : Cases) {
        ...
        MedFunc Caller = makeMergedExactAddressReturn(
            Kind, TargetArch, FirstInteriorVA, SecondInteriorVA);
        replaceSecondMergedAddressArm(Caller, Kind, Current.Replacement);
        MedFunc Owner = makeInteriorCodeIdentityOwner(
            "invalid_arm_blockaddress_target", CodeVA, FirstInteriorVA,
            SecondInteriorVA);
        llvm::LLVMContext Context;
        testing::internal::CaptureStderr();
        auto Module = MedLLVMEmitter().emit({Caller, Owner}, Context,
...
TEST(LLVMCodePointerInvariantBoundary,
     RejectsCrossOwnerBlockAddressesAcrossValueMerges) {
  constexpr Arch TargetArch = Arch::X64;
  constexpr va_t FirstInteriorVA = CodeVA + 0x10;
  constexpr va_t SecondOwnerEntryVA = CodeVA + 0x40;
  constexpr va_t SecondInteriorVA = CodeVA + 0x50;
  for (BinaryFormat Format :
       {BinaryFormat::MachO, BinaryFormat::ELF, BinaryFormat::COFF})
    for (ExactAddressMergeKind Kind :
         {ExactAddressMergeKind::Phi, ExactAddressMergeKind::Select}) {
      ...
      MedFunc FirstOwner = makeInteriorCodeIdentityOwner(
          "first_blockaddress_owner", CodeVA, FirstInteriorVA, CodeVA + 0x20);
      MedFunc SecondOwner = makeInteriorCodeIdentityOwner(
          "second_blockaddress_owner", SecondOwnerEntryVA, SecondInteriorVA,
          CodeVA + 0x60);
      llvm::LLVMContext Context;
      testing::internal::CaptureStderr();
      auto Module =
...
TEST(LLVMCodePointerInvariantBoundary,
     RejectsConflictingLiftedBlockOwnerAcrossValueMerges) {
  constexpr Arch TargetArch = Arch::X64;
  constexpr va_t ConflictingInteriorVA = CodeVA + 0x10;
  for (BinaryFormat Format :
       {BinaryFormat::MachO, BinaryFormat::ELF, BinaryFormat::COFF})
    for (ExactAddressMergeKind Kind :
         {ExactAddressMergeKind::Phi, ExactAddressMergeKind::Select}) {
      ...
      SCOPED_TRACE(Kind == ExactAddressMergeKind::Phi ? "phi" : "select");
      BinaryImage Image = makeSpilledConstTableImage(TargetArch, Format);
      ASSERT_TRUE(Image.isCodeAddress(ConflictingInteriorVA));
