...
static fs::path i386StackPICObj() {
  return fs::path(TEST_OBJ_DIR) / "test_jumptable_stack_loc8_pic.o";
}

TEST_F(JTE_X86_32, StackMemcpyTableNeedsDedicatedEvidenceCeiling) {
  auto ImageOrErr = neverd::loadBinary(i386StackPICObj());
  ASSERT_TRUE(static_cast<bool>(ImageOrErr))
      << llvm::toString(ImageOrErr.takeError());
  neverd::BinaryImage &Image = *ImageOrErr;
  const neverd::Symbol *Function =
      Image.findSymbol("jt_i386_stack_loc8_budget");
  ...
  auto Recovered = [](const neverd::LowFunc &Low) {
    return Low.JumpTables.size() == 1 &&
           Low.JumpTables.front().Targets.size() == 8;
  };

  const neverd::LowFunc Full =
      BuildWithBudget(neverd::limits::kMaxJumpTableStackEvidenceWork);
  ASSERT_TRUE(Recovered(Full));
  const neverd::va_t Branch = Full.JumpTables.front().InsnAddr;

  size_t Lo = 1;
  size_t Hi = neverd::limits::kMaxJumpTableStackEvidenceWork;
  while (Lo < Hi) {
  ...
  EXPECT_GT(MinimumSuccessfulBudget, neverd::limits::kMaxJumpTableEvidenceWork);
...
TEST_F(JTE_X86_32, GOTPCModelRequiresLifterAuthenticatedCallPopSeed) {
  ...
  const neverd::LowOp *SeedCopy = findExactLowOp(
      Low, GetPc.InstructionAddr, GetPc.OpSeq, GetPc.OutputOpcode);
...
TEST_F(JTE_X86_32, GOTPCModelRejectsAddressTakenPopEntry) {
  ...
  ASSERT_TRUE(static_cast<bool>(ImageOrErr))
      << llvm::toString(ImageOrErr.takeError());
  neverd::BinaryImage &Image = *ImageOrErr;
  const neverd::Symbol *Function = Image.findSymbol("jt_i386_gotpc_rooted_pop");
  ...
  const neverd::LowFunc Low =
      Builder.build(Image, Decoder, Function->Addr, "jt_i386_gotpc_rooted_pop");
...
TEST_F(JTE_X86_32, GOTOFFSwitchRequiresExactCallPopAndDataFieldOccurrences) {
  ...
  const neverd::Symbol *Table = Image.findSymbol("jt_i386_gotoff_switch_table");
  ...
  for (const auto &[FieldVA, Field] : Image.DataAddressRelocOperands)
    if (FieldVA >= Positive->Addr &&
        ...
        Field.Width == Image.getPointerSize() && Field.TargetVA == Table->Addr)
  ...
  EXPECT_TRUE(Recover(*Positive).JumpTables.empty())
      << "a GOTOFF field without an authenticated target owner must fail "
         "closed";

  Image.DataAddressRelocOperands.at(FieldVA) = SavedField;
  ASSERT_EQ(SavedField.Kind, neverd::RelocatedAddressFieldKind::I386ELFGOTOFF);
...
TEST_F(JTE_X86_32, GOTOFFStackLoopKeepsDistinctTableDomains) {
  auto ImageOrErr = neverd::loadBinary(i386GOTOFFStackGroupObj());
  ASSERT_TRUE(static_cast<bool>(ImageOrErr))
      << llvm::toString(ImageOrErr.takeError());
  const auto &Image = *ImageOrErr;
  const auto *Function = Image.findSymbol("jt_i386_gotoff_two_stack_tables");
  const auto *First = Image.findSymbol("jt_i386_gotoff_two_stack_first_table");
  const auto *Second =
      Image.findSymbol("jt_i386_gotoff_two_stack_second_table");
  const auto *FirstBranch =
      Image.findSymbol("jt_i386_gotoff_two_stack_first_branch");
  const auto *SecondBranch =
      Image.findSymbol("jt_i386_gotoff_two_stack_second_branch");
  ASSERT_NE(Function, nullptr);
  ASSERT_NE(First, nullptr);
  ASSERT_NE(Second, nullptr);
  ASSERT_NE(FirstBranch, nullptr);
  ASSERT_NE(SecondBranch, nullptr);
  ASSERT_EQ(Second->Addr, First->Addr + 28);
  for (unsigned Slot = 0; Slot != 14; ++Slot)
    ASSERT_TRUE(Image.CodePtrRelocSlots.count(First->Addr + 4 * Slot));
  ...
  for (const auto &Table : Low.JumpTables) {
    EXPECT_TRUE(Bases.insert(Table.BaseAddr).second);
    if (Table.InsnAddr == FirstBranch->Addr)
      EXPECT_EQ(Table.BaseAddr, First->Addr);
    else {
      EXPECT_EQ(Table.InsnAddr, SecondBranch->Addr);
      EXPECT_EQ(Table.BaseAddr, Second->Addr);
    }
    EXPECT_TRUE(Table.HasDispatchSlotMap);
    EXPECT_EQ(Table.SlotIndices, (std::vector<uint32_t>{0, 1, 2, 3, 4, 5, 6}));
    // LowIR represents dense identity labels implicitly with an empty vector.
    // Explicit labels must describe the same domain for each separate table.
    if (!Table.CaseLabels.empty())
      EXPECT_EQ(Table.CaseLabels, (std::vector<int64_t>{0, 1, 2, 3, 4, 5, 6}));
    ASSERT_EQ(Table.Targets.size(), 7u);
  ...
  EXPECT_EQ(Bases, (std::set<neverd::va_t>{First->Addr, Second->Addr}));
  EXPECT_TRUE(Low.UnsafeIndirectBranchAddresses.empty());
  EXPECT_FALSE(Builder.hasProvisionalRelativeEdgesForTesting());
  EXPECT_FALSE(lowFunctionHasOpcode(Low, neverd::NdOp::INDIR_CALL));
}

TEST_F(JTE_X86_32, GOTOFFStackLoopRejectsUninitializedModelAndStaleGuard) {
  for (unsigned Variant = 0; Variant != 4; ++Variant) {
    SCOPED_TRACE(Variant);
    const bool StaleGuard = Variant != 0;
    auto ImageOrErr = neverd::loadBinary(i386GOTOFFStackGroupObj());
    ASSERT_TRUE(static_cast<bool>(ImageOrErr))
        << llvm::toString(ImageOrErr.takeError());
    auto &Image = *ImageOrErr;
    const auto *Function = Image.findSymbol("jt_i386_gotoff_two_stack_tables");
    const auto *Branch = Image.findSymbol(
        Variant == 2 ? "jt_i386_gotoff_two_stack_second_branch"
                     : "jt_i386_gotoff_two_stack_first_branch");
    const std::array<const char *, 4> MutationNames{
        "jt_i386_gotoff_two_stack_got_store",
        "jt_i386_gotoff_two_stack_first_guard",
        "jt_i386_gotoff_two_stack_second_guard",
        "jt_i386_gotoff_two_stack_first_guard"};
    const auto *Mutation = Image.findSymbol(MutationNames[Variant]);
    ASSERT_NE(Function, nullptr);
    ASSERT_NE(Branch, nullptr);
    ASSERT_NE(Mutation, nullptr);
    const std::array<uint8_t, 3> Expected =
        StaleGuard ? std::array<uint8_t, 3>{0x83, 0xf8, 0x06}
                   : std::array<uint8_t, 3>{0x89, 0x45, 0xfc};
    const uint8_t *Bytes = Image.readVA(Mutation->Addr, Expected.size());
    ASSERT_NE(Bytes, nullptr);
    ASSERT_TRUE(std::equal(Expected.begin(), Expected.end(), Bytes));
    // SUB sets the same unsigned guard flags as CMP, but changes the LOAD's
    // index from k to k-6. The old [0,6] bound is not a bound on that index.
    const std::array<uint8_t, 3> Replacement =
        Variant == 3 ? std::array<uint8_t, 3>{0x83, 0xf8, 0x07}
        : StaleGuard ? std::array<uint8_t, 3>{0x83, 0xe8, 0x06}
                     : std::array<uint8_t, 3>{0x90, 0x90, 0x90};
    ASSERT_TRUE(
        Image.writeVA(Mutation->Addr, Replacement.data(), Replacement.size()));
    neverd::Decoder Decoder;
    ASSERT_TRUE(Decoder.init(Image.Arch, Image.Mode));
    neverd::CFGBuilder Builder;
    const auto Low =
    ...
    EXPECT_TRUE(std::none_of(
        ...
        [&](const auto &Table) { return Table.InsnAddr == Branch->Addr; }));
    // Failure of the second member must not publish the successful prefix of
    // a transaction that relied on both tables' provisional root removal.
    if (Variant != 1)
...
TEST_F(JTE_X86_32, GOTOFFStackLoopRetainsAdjacentIndependentPointer) {
  auto ImageOrErr = neverd::loadBinary(i386GOTOFFStackGroupObj());
  ASSERT_TRUE(static_cast<bool>(ImageOrErr))
      << llvm::toString(ImageOrErr.takeError());
  auto &Image = *ImageOrErr;
  ...
  const auto FirstAddr = First->Addr;
  const auto *Owner = Image.getSectionFor(FirstAddr);
  ASSERT_NE(Owner, nullptr);
  ASSERT_EQ(Owner->VA, FirstAddr);
  ASSERT_EQ(Owner->Size, 56u);
  const auto ExtraSlot = FirstAddr + Owner->Size;
  const uint8_t *TargetBytes = Image.readVA(FirstAddr, 4);
  ASSERT_NE(TargetBytes, nullptr);
  const std::array<uint8_t, 4> Pointer{TargetBytes[0], TargetBytes[1],
                                       TargetBytes[2], TargetBytes[3]};
  size_t Changed = 0;
  for (auto &Segment : Image.Segments) {
    if (!Segment.contains(FirstAddr))
      continue;
    ASSERT_EQ(Segment.VA + Segment.Size, ExtraSlot);
    ASSERT_EQ(Segment.Data.size(), Segment.Size);
    Segment.Data.insert(Segment.Data.end(), Pointer.begin(), Pointer.end());
    Segment.Size += Pointer.size();
    Segment.FileSz += Pointer.size();
    ++Changed;
  }
  ASSERT_EQ(Changed, 1u);
  for (auto &Section : Image.Sections)
    if (Section.VA == FirstAddr) {
      Section.Size += Pointer.size();
      Section.FileSz += Pointer.size();
    }
  neverd::Symbol Independent;
  Independent.Name = "independent_case_pointer";
  Independent.Addr = ExtraSlot;
  Independent.Size = Pointer.size();
  Image.Symbols.push_back(std::move(Independent));
  ASSERT_TRUE(Image.CodePtrRelocSlots.insert(ExtraSlot).second);
  // The same physical section is not ownership permission for this separate
  // pointer object. Its reference keeps the shared case an independent root.
  neverd::Decoder Decoder;
  ASSERT_TRUE(Decoder.init(Image.Arch, Image.Mode));
  neverd::CFGBuilder Builder;
  const auto Low = Builder.build(Image, Decoder, FunctionAddr, FunctionName);
  ASSERT_EQ(Low.RelocatedInstructionScalarModelOccurrences.size(), 1u);
  EXPECT_TRUE(Builder.i386GOTOFFGraphQueryIssuedForTesting());
  EXPECT_TRUE(Low.JumpTables.empty());
  EXPECT_FALSE(Builder.hasProvisionalRelativeEdgesForTesting());
...
TEST_F(JTE_X86_32, GOTOFFStackLoopRetainsReaderAfterSecondDispatch) {
  auto ImageOrErr = neverd::loadBinary(i386GOTOFFStackGroupObj());
  ASSERT_TRUE(static_cast<bool>(ImageOrErr))
      << llvm::toString(ImageOrErr.takeError());
  const auto &Image = *ImageOrErr;
  const auto *Function =
      Image.findSymbol("jt_i386_gotoff_consumer_stack_tables");
  ASSERT_NE(Function, nullptr);
  neverd::Decoder Decoder;
  ASSERT_TRUE(Decoder.init(Image.Arch, Image.Mode));
  neverd::CFGBuilder Builder;
...
TEST_F(JTE_X86_32, GOTOFFStackLoopDiscardsGroupOnEvidenceExhaustion) {
  auto ImageOrErr = neverd::loadBinary(i386GOTOFFStackGroupObj());
  ASSERT_TRUE(static_cast<bool>(ImageOrErr))
      << llvm::toString(ImageOrErr.takeError());
  const auto &Image = *ImageOrErr;
  const auto *Function = Image.findSymbol("jt_i386_gotoff_two_stack_tables");
  ASSERT_NE(Function, nullptr);
  for (size_t Budget : {size_t{0}, size_t{30000000}, size_t{60000000}}) {
    SCOPED_TRACE(Budget);
    neverd::Decoder Decoder;
    ASSERT_TRUE(Decoder.init(Image.Arch, Image.Mode));
    neverd::CFGBuilder Builder;
...
TEST_F(JTE_X86_32, GOTOFFStackLoopFailsClosedOnDedicatedStackBudget) {
  auto ImageOrErr = neverd::loadBinary(i386GOTOFFStackGroupObj());
  ASSERT_TRUE(static_cast<bool>(ImageOrErr))
      << llvm::toString(ImageOrErr.takeError());
  const auto &Image = *ImageOrErr;
  const auto *Function = Image.findSymbol("jt_i386_gotoff_two_stack_tables");
  const auto *First = Image.findSymbol("jt_i386_gotoff_two_stack_first_branch");
  const auto *Second =
      Image.findSymbol("jt_i386_gotoff_two_stack_second_branch");
  ASSERT_NE(Function, nullptr);
  ASSERT_NE(First, nullptr);
  ...
  for (size_t Budget : {size_t{0}, size_t{1}}) {
    ...
    ASSERT_TRUE(Decoder.init(Image.Arch, Image.Mode));
    neverd::CFGBuilder Builder;
    // Keep the stage and candidate budgets unchanged: this exercises the
    // dedicated frame-analysis allowance used by the stack-loaded GOT base.
    Builder.setStackTableEvidenceBudgetForTesting(Budget);
    const auto Low =
        Builder.build(Image, Decoder, Function->Addr, Function->Name);
    EXPECT_TRUE(Low.JumpTables.empty());
    EXPECT_FALSE(Builder.hasProvisionalRelativeEdgesForTesting());
    EXPECT_FALSE(lowFunctionHasOpcode(Low, neverd::NdOp::INDIR_CALL));
    for (neverd::va_t Branch : {First->Addr, Second->Addr}) {
      SCOPED_TRACE(Branch);
      bool Retained = false;
      for (const auto &Block : Low.Blocks)
        for (const auto &Op : Block.Ops)
          Retained |= Op.Addr == Branch && Op.Opcode == neverd::NdOp::INDIR_BR;
      EXPECT_TRUE(Retained);
    }
  }
}

static void verifyGOTOFFStackGroupPublicationLifecycle(bool RevokeOwner) {
  auto ImageOrErr = neverd::loadBinary(i386GOTOFFStackGroupObj());
  ASSERT_TRUE(static_cast<bool>(ImageOrErr))
      << llvm::toString(ImageOrErr.takeError());
  const auto &Image = *ImageOrErr;
  ...
  const auto *SecondBranch =
      Image.findSymbol("jt_i386_gotoff_two_stack_second_branch");
  const auto *FirstTable =
      Image.findSymbol("jt_i386_gotoff_two_stack_first_table");
  const auto *SecondTable =
      Image.findSymbol("jt_i386_gotoff_two_stack_second_table");
  ASSERT_NE(Function, nullptr);
  ASSERT_NE(FirstBranch, nullptr);
  ASSERT_NE(SecondBranch, nullptr);
  ASSERT_NE(FirstTable, nullptr);
  ASSERT_NE(SecondTable, nullptr);

  neverd::Decoder Decoder;
  ASSERT_TRUE(Decoder.init(Image.Arch, Image.Mode));
  neverd::CFGBuilder Builder;
  if (RevokeOwner)
    Builder.setRevokePublishedJumpTableGroupOwnerForTesting(true);
  else
    Builder.setExhaustPublishedJumpTableGroupCommitTailForTesting(true);
  const auto Low =
      Builder.build(Image, Decoder, Function->Addr, Function->Name);
  const auto &State = Builder.jumpTableGroupLifecycleStateForTesting();
  ASSERT_EQ(State.PublishedMemberCount, 2u);
  if (RevokeOwner) {
    EXPECT_TRUE(State.OwnerRevocationInjected);
    EXPECT_EQ(State.OwnerCountBeforeRevocation, 2u);
    EXPECT_TRUE(State.OwnerRevocationClearedAllMembers);
    EXPECT_FALSE(State.CommitTailInjected);
    // Losing a suppression owner rejects this group for the rest of the
    // build. The real closure must remove both members and retain their
    // unresolved-jump identities rather than retrying the rejected proof.
    EXPECT_TRUE(Low.JumpTables.empty());
    EXPECT_FALSE(Builder.hasProvisionalRelativeEdgesForTesting());
    EXPECT_FALSE(lowFunctionHasOpcode(Low, neverd::NdOp::INDIR_CALL));
    for (neverd::va_t Branch : {FirstBranch->Addr, SecondBranch->Addr}) {
      SCOPED_TRACE(Branch);
      EXPECT_EQ(Low.EverPublishedJumpTableBranchAddresses.count(Branch), 1u);
    ...
    EXPECT_TRUE(State.CommitTailInjected);
    EXPECT_TRUE(State.CommitTailRollbackClearedAllMembers);
    EXPECT_FALSE(State.OwnerRevocationInjected);
  }

  // Recovery after the one-shot resource fault does not replace the
  // intermediate cleanup assertions above: both the rollback and the replay
  // must succeed.
  ASSERT_EQ(Low.JumpTables.size(), 2u);
  std::set<neverd::va_t> Dispatches;
  for (const auto &Table : Low.JumpTables) {
    EXPECT_TRUE(Dispatches.insert(Table.InsnAddr).second);
    if (Table.InsnAddr == FirstBranch->Addr)
      EXPECT_EQ(Table.BaseAddr, FirstTable->Addr);
    else {
      EXPECT_EQ(Table.InsnAddr, SecondBranch->Addr);
      EXPECT_EQ(Table.BaseAddr, SecondTable->Addr);
    }
    EXPECT_TRUE(Table.HasDispatchSlotMap);
    EXPECT_EQ(Table.SlotIndices, (std::vector<uint32_t>{0, 1, 2, 3, 4, 5, 6}));
    if (!Table.CaseLabels.empty())
  ...
  EXPECT_EQ(Dispatches,
            (std::set<neverd::va_t>{FirstBranch->Addr, SecondBranch->Addr}));
  EXPECT_TRUE(Low.UnsafeIndirectBranchAddresses.empty());
  EXPECT_FALSE(Builder.hasProvisionalRelativeEdgesForTesting());
  EXPECT_FALSE(lowFunctionHasOpcode(Low, neverd::NdOp::INDIR_CALL));
}

TEST_F(JTE_X86_32, GOTOFFStackLoopReplaysAfterPublishedCommitTailExhaustion) {
  ASSERT_NO_FATAL_FAILURE(
      verifyGOTOFFStackGroupPublicationLifecycle(/*RevokeOwner=*/false));
}

TEST_F(JTE_X86_32, GOTOFFStackLoopRevokesWholeGroupWhenOneOwnerIsLost) {
  ASSERT_NO_FATAL_FAILURE(
      verifyGOTOFFStackGroupPublicationLifecycle(/*RevokeOwner=*/true));
}
...
TEST_F(JTE_X86_32, DirectGOTOFFGroupsRetainDistinctTableBases) {
  ...
  for (bool Separate : {false, true}) {
    const std::string Name =
        Separate ? "jt_i386_direct_separate" : "jt_i386_direct_shared";
    SCOPED_TRACE(Name);
    const auto *Function = Image.findSymbol(Name);
    const auto *Table = Image.findSymbol(Name + "_table");
    ASSERT_NE(Function, nullptr);
    ASSERT_NE(Table, nullptr);
    ASSERT_EQ(Table->Size, 16u);
    std::set<neverd::va_t> ExpectedBases{Table->Addr};
    if (Separate) {
      const auto *Adjacent = Image.findSymbol(Name + "_adjacent");
      ASSERT_NE(Adjacent, nullptr);
      ASSERT_EQ(Adjacent->Addr, Table->Addr + Table->Size);
      ASSERT_EQ(Adjacent->Size, 16u);
      ExpectedBases.insert(Adjacent->Addr);
    }
    neverd::Decoder Decoder;
    ASSERT_TRUE(Decoder.init(Image.Arch, Image.Mode));
    neverd::CFGBuilder Builder;
    ...
    ASSERT_EQ(Low.JumpTables.size(), 2u);
    std::set<neverd::va_t> Bases;
    for (const auto &TableInfo : Low.JumpTables) {
      Bases.insert(TableInfo.BaseAddr);
      if (Separate) {
        EXPECT_EQ(TableInfo.AuthenticatedTableLoads.size(), 1u);
        EXPECT_EQ(TableInfo.Targets.size(), 2u);
      } else {
        EXPECT_EQ(TableInfo.BaseAddr, Table->Addr);
        EXPECT_EQ(TableInfo.Targets.size(), 4u);
      }
    }
    EXPECT_EQ(Bases, ExpectedBases);
  }
}
...
TEST_F(JTE_X86_32, GOTOFFModuloPrefixReplaysEveryDispatchArm) {
  ...
  ASSERT_TRUE(static_cast<bool>(ImageOrErr))
      << llvm::toString(ImageOrErr.takeError());
  const auto &Image = *ImageOrErr;
  for (const char *Name : {"jt_i386_prefix_modulo", "jt_i386_prefix_bad_magic",
                           "jt_i386_prefix_bad_arm"}) {
    SCOPED_TRACE(Name);
    const auto *Function = Image.findSymbol(Name);
    const auto *Table = Image.findSymbol(std::string(Name) + "_table");
    ASSERT_NE(Function, nullptr);
    ASSERT_NE(Table, nullptr);
    ...
    neverd::CFGBuilder Builder;
    const auto Low = Builder.build(Image, Decoder, Function->Addr, Name);
    if (std::string_view(Name) == "jt_i386_prefix_modulo") {
      ASSERT_EQ(Low.JumpTables.size(), 1u);
      EXPECT_EQ(Low.JumpTables.front().Targets.size(), 12u);
      EXPECT_EQ(Low.JumpTables.front().BaseAddr, Table->Addr);
      EXPECT_TRUE(Low.UnsafeIndirectBranchAddresses.empty());
    } else {
      EXPECT_TRUE(Low.JumpTables.empty());
    }
...
TEST_F(JTE_X86_32,
  ...
  ASSERT_EQ(Read32(OverlapField->Addr),
            std::optional<uint32_t>(static_cast<uint32_t>(OverlapTarget->Addr)))
      << "ambiguity suppresses provenance, not the supported GOTOFF write";
  ASSERT_EQ(
      Read32(NonOverlapField->Addr),
      std::optional<uint32_t>(static_cast<uint32_t>(NonOverlapTarget->Addr)));
...
TEST_F(JTE_X86_32, SameI386FieldWithMultipleValueRelocationsStaysOpaque) {
  auto ImageOrErr = neverd::loadBinary(i386GOTPCModelObj());
  ASSERT_TRUE(static_cast<bool>(ImageOrErr))
      << llvm::toString(ImageOrErr.takeError());
  neverd::BinaryImage &Image = *ImageOrErr;
  ...
  const neverd::Symbol *Bias = Image.findSymbol("jt_i386_gotpc_conflict_bias");
  const neverd::Symbol *TextBase = Image.findSymbol("jt_i386_gotpc_text_base");
  const neverd::Symbol *AmbiguousBranch =
      Image.findSymbol("jt_i386_gotoff_same_field_conflict_ambiguous_branch");
  const neverd::Symbol *CallbackBranch =
      Image.findSymbol("jt_i386_gotoff_same_field_conflict_callback_branch");
  const neverd::Symbol *ValidBranch =
      Image.findSymbol("jt_i386_gotoff_same_field_conflict_valid_branch");
  const neverd::Symbol *ZeroBase =
      Image.findSymbol("jt_i386_gotoff_same_field_conflict_zero_base");
  ASSERT_NE(Function, nullptr);
  ...
  const uint32_t LLDModelZeroEncoded =
      static_cast<uint32_t>(TextBase->Addr - PC->Addr - Bias->Addr);
  ...
  for (const neverd::LowBlock &Block : Low.Blocks)
    for (const neverd::LowOp &Op : Block.Ops) {
      ...
      for (unsigned I = 0; I < Op.NumInputs; ++I) {
        ...
        HasLLDImmediate |=
            Input.isConst() && Input.Size == 4 &&
            static_cast<uint32_t>(Input.Offset) == LLDModelZeroEncoded;
      }
      if (HasLLDImmediate)
        DecodedAdd = &Op;
    }
  ...
  const bool ZeroBaseDecoded = std::any_of(
      Low.Blocks.begin(), Low.Blocks.end(), [&](const neverd::LowBlock &Block) {
        return std::any_of(
            Block.Ops.begin(), Block.Ops.end(),
            [&](const neverd::LowOp &Op) { return Op.Addr == ZeroBase->Addr; });
      });
  ...
  EXPECT_EQ(Low.UnsafeIndirectBranchAddresses.count(AmbiguousBranch->Addr), 1u);
  EXPECT_EQ(
      Low.EverPublishedJumpTableBranchAddresses.count(AmbiguousBranch->Addr),
      0u)
...
TEST_F(JTE_X86_32,
  ...
  const neverd::Symbol *Callback =
      Image.findSymbol("jt_i386_gotoff_displacement_conflict_callback_branch");
  ...
  LookupExhaustedBuilder.setIncompleteBranchMarkerEvidenceBudgetForTesting(6);
  ...
  EXPECT_EQ(LookupExhaustedBuilder.i386GOTOFFTombstoneLookupCountForTesting(),
            0u)
  ...
  EXPECT_TRUE(LookupExhaustedHasOpcodeAt(Branch->Addr, neverd::NdOp::INDIR_BR));
  EXPECT_FALSE(
      LookupExhaustedHasOpcodeAt(Branch->Addr, neverd::NdOp::INDIR_CALL));
  EXPECT_EQ(LookupExhausted.UnsafeIndirectBranchAddresses.count(Branch->Addr),
            1u);
}
...
TEST_F(JTE_X86_32, I386GOTPCReplayRetiresOnlyTheExactQueryKey) {
  ...
  const Key SameBranchFirst = std::make_tuple(Branch, 0x1030, 6, 0, 0x5000);
  const Key SameBranchSecond = std::make_tuple(Branch, 0x1040, 8, 1, 0x6000);
  const Key Other = std::make_tuple(OtherBranch, 0x4010, 2, 0, 0x7000);
...
TEST_F(JTE_X86_32, I386GOTOFFProposalRootCacheSeparatesSiblingCandidates) {
...
TEST(TargetRoleProofContextKeyTest,
  ...
  EXPECT_FALSE(neverd::detail::targetRoleProofContextMatches(
      ...
      /*HasActiveProofRoots=*/true, /*ConsumerAuditMode=*/true, DifferentRoots))
      << "equal-sized but different proof-root sets must not share a proof";
}

TEST_F(JTE_X86_32, AmbiguousI386GOTPCCarrySurvivesUnreplayedStableGraph) {
  auto ImageOrErr = neverd::loadBinary(i386GOTPCModelObj());
  ASSERT_TRUE(static_cast<bool>(ImageOrErr))
      << llvm::toString(ImageOrErr.takeError());
  neverd::BinaryImage &Image = *ImageOrErr;
  ...
  const neverd::Symbol *AmbiguousBranch =
      Image.findSymbol("jt_i386_gotoff_ambiguous_late_shape_loss_branch");
  const neverd::Symbol *ValidBranch =
      Image.findSymbol("jt_i386_gotoff_ambiguous_late_shape_loss_valid_branch");
  ...
  const bool LateEdgeDecoded = std::any_of(
      Low.Blocks.begin(), Low.Blocks.end(), [&](const neverd::LowBlock &Block) {
        return std::any_of(
            Block.Ops.begin(), Block.Ops.end(),
            [&](const neverd::LowOp &Op) { return Op.Addr == LateEdge->Addr; });
      });
...
TEST_F(JTE_X86_32, AmbiguousI386GOTPCReplaySurvivesCommitTailExhaustion) {
  auto ImageOrErr = neverd::loadBinary(i386GOTPCModelObj());
  ASSERT_TRUE(static_cast<bool>(ImageOrErr))
      << llvm::toString(ImageOrErr.takeError());
  neverd::BinaryImage &Image = *ImageOrErr;
  ...
  auto BuildWithBudget = [&](size_t Budget,
    ...
    Result.PendingCarry = Builder.hasPendingI386GOTPCAmbiguityForTesting();
    return Result;
  };
  auto HasOpcodeAt = [](const neverd::LowFunc &Low, neverd::va_t Addr,
                        neverd::NdOp Opcode) {
  ...
  auto IsComplete = [&](const BudgetedBuild &Result) {
    const auto Valid = std::find_if(
        ...
        [&](const auto &Table) { return Table.InsnAddr == ValidBranch->Addr; });
    return Valid != Result.Low.JumpTables.end() && Valid->Targets.size() == 2 &&
  ...
  EXPECT_TRUE(
      HasOpcodeAt(Boundary.Low, AmbiguousBranch->Addr, neverd::NdOp::INDIR_BR));
  EXPECT_FALSE(HasOpcodeAt(Boundary.Low, AmbiguousBranch->Addr,
                           neverd::NdOp::INDIR_CALL));
  EXPECT_EQ(
      Boundary.Low.UnsafeIndirectBranchAddresses.count(AmbiguousBranch->Addr),
      1u);
  ...
  EXPECT_TRUE(HasOpcodeAt(Boundary.Low, CallbackBranch->Addr,
                          neverd::NdOp::INDIR_CALL));
  EXPECT_FALSE(
      HasOpcodeAt(Boundary.Low, CallbackBranch->Addr, neverd::NdOp::INDIR_BR));
  EXPECT_EQ(
      Boundary.Low.UnsafeIndirectBranchAddresses.count(CallbackBranch->Addr),
      0u);
  EXPECT_TRUE(
      HasOpcodeAt(Boundary.Low, ValidBranch->Addr, neverd::NdOp::INDIR_BR));
  EXPECT_FALSE(
      HasOpcodeAt(Boundary.Low, ValidBranch->Addr, neverd::NdOp::INDIR_CALL));
  ...
  EXPECT_EQ(Boundary.Low.UnsafeIndirectBranchAddresses.count(ValidBranch->Addr),
            0u);
  ...
  EXPECT_EQ(
      Complete.Low.UnsafeIndirectBranchAddresses.count(AmbiguousBranch->Addr),
      1u);
  EXPECT_EQ(
      Complete.Low.UnsafeIndirectBranchAddresses.count(CallbackBranch->Addr),
      0u);
...
TEST_F(JTE_X86_32, AbsoluteDataRelocationCannotMasqueradeAsGOTOFF) {
  ...
  const neverd::Symbol *Table = Image.findSymbol("jt_i386_abs32_switch_table");
  ASSERT_NE(Function, nullptr);
  ASSERT_NE(Table, nullptr);
  ...
  EXPECT_EQ(Fields.front().Kind, neverd::RelocatedAddressFieldKind::Generic);
...
TEST_F(JTE_X86_32, GOTOFFSwitchRequiresTheExactScalarModelOrigin) {
  ...
  const neverd::LowFunc Computed = Recover("jt_i386_gotoff_literal_zero_base");
...
TEST_F(JTE_X86_32, GOTOFFProposalBudgetExhaustionFailsClosed) {
  ...
  auto BuildWithBudget = [&](size_t Budget) {
    ...
    BudgetedBuild Result;
    Result.Low = Builder.build(Image, Decoder, Function->Addr,
                               "jt_i386_gotoff_switch_call_pop");
    Result.GraphQueryIssued = Builder.i386GOTOFFGraphQueryIssuedForTesting();
...
struct COFFImageRelativeRVASwitchOptions {
  bool ScalarStorageBase = false;
  bool ScalarTargetBase = false;
  bool UseNonParameterIndex = false;
  bool RelocatedTableSlot = false;
};

static neverd::BinaryImage
makeCOFFImageRelativeRVASwitch(COFFImageRelativeRVASwitchOptions Options = {}) {
  ...
  constexpr size_t TableOffset = TableVA - FunctionVA;
  ...
  Text.Name = ".text";
  Text.VA = FunctionVA;
  Text.Size = TableOffset + 6 * sizeof(uint32_t);
  Text.Flags =
      neverd::SegmentFlags::Readable | neverd::SegmentFlags::Executable;
  ...
  auto AppendU32 = [&](uint32_t Value) {
    for (unsigned Byte = 0; Byte < sizeof(Value); ++Byte)
      DispatchBytes.push_back(static_cast<uint8_t>(Value >> (Byte * 8)));
  };
  ...
  auto AppendImageBaseLEA = [&](uint8_t REX, uint8_t ModRM) {
    const neverd::va_t NextInstruction = FunctionVA + DispatchBytes.size() + 7;
    const int64_t Displacement =
        static_cast<int64_t>(ImageBase) - static_cast<int64_t>(NextInstruction);
    EXPECT_GE(Displacement, std::numeric_limits<int32_t>::min());
    EXPECT_LE(Displacement, std::numeric_limits<int32_t>::max());
    DispatchBytes.insert(DispatchBytes.end(), {REX, 0x8d, ModRM});
    AppendU32(static_cast<uint32_t>(Displacement));
  };

  if (!Options.UseNonParameterIndex)
    DispatchBytes.insert(DispatchBytes.end(),
                         {0x89, 0xc9}); // mov ecx, ecx; defines all of rcx
  DispatchBytes.insert(
      DispatchBytes.end(),
      {0x83, Options.UseNonParameterIndex ? uint8_t{0xf8} : uint8_t{0xf9},
       0x05}); // cmp eax/ecx, 5
  const size_t BranchNextOffset = DispatchBytes.size() + 6;
  EXPECT_LE(BranchNextOffset, size_t{0x40});
  DispatchBytes.insert(DispatchBytes.end(), {0x0f, 0x87});
  AppendU32(static_cast<uint32_t>(0x40 - BranchNextOffset)); // ja default
  if (Options.ScalarStorageBase) {
    DispatchBytes.insert(DispatchBytes.end(), {0x48, 0xba});
    AppendU64(ImageBase); // movabs rdx, image base
  } else {
    AppendImageBaseLEA(0x48, 0x15); // lea rdx, image base
  }
  DispatchBytes.insert(
      DispatchBytes.end(),
      {0x8b, 0x8c, Options.UseNonParameterIndex ? uint8_t{0x82} : uint8_t{0x8a},
       0x00, 0x11, 0x00, 0x00}); // mov ecx,[rdx+rax/rcx*4+1100h]
  if (Options.ScalarTargetBase) {
    DispatchBytes.insert(DispatchBytes.end(), {0x49, 0xb8});
    AppendU64(ImageBase); // movabs r8, image base
  } else {
    AppendImageBaseLEA(0x4c, 0x05); // lea r8, image base
  }
  DispatchBytes.insert(DispatchBytes.end(), {0x49, 0x03, 0xc8, // add rcx, r8
                                             0xff, 0xe1});     // jmp rcx
  EXPECT_LE(DispatchBytes.size(), size_t{0x30});
  std::copy(DispatchBytes.begin(), DispatchBytes.end(), Text.Data.begin());

  const std::array<uint32_t, 6> TargetRVAs{0x1030, 0x1032, 0x1034,
                                           0x1036, 0x1038, 0x103a};
  for (size_t I = 0; I < TargetRVAs.size(); ++I) {
    Text.Data[0x30 + I * 2] = 0xc3;
    const uint32_t RVA = TargetRVAs[I];
    for (unsigned Byte = 0; Byte < sizeof(RVA); ++Byte)
      Text.Data[TableOffset + I * sizeof(RVA) + Byte] =
  ...
  TextSection.Name = ".text";
  TextSection.VA = FunctionVA;
  TextSection.Size = TableOffset + 6 * sizeof(uint32_t);
  TextSection.Flags =
      neverd::SegmentFlags::Readable | neverd::SegmentFlags::Executable;
  Image.Sections.push_back(std::move(TextSection));
  if (Options.RelocatedTableSlot)
    Image.Relocations.push_back(neverd::RelocationEntry{.Address = TableVA});

  neverd::Symbol Function = neverd::Symbol::makeFunc(FunctionVA, 0x41);
  Function.Name = "coff_image_relative_rva_switch";
  Image.Symbols.push_back(std::move(Function));
  Image.KnownCodeRanges.emplace_back(FunctionVA, FunctionVA + 0x41);
  return Image;
}
...
TEST_F(JTE_X86_64, RecoversExactCOFFImageRelativeRVASwitch) {
  ...
  const std::vector<neverd::va_t> ExpectedTargets{
      ImageBase + 0x1030, ImageBase + 0x1032, ImageBase + 0x1034,
      ImageBase + 0x1036, ImageBase + 0x1038, ImageBase + 0x103a};

  auto Recover = [&](COFFImageRelativeRVASwitchOptions Options = {}) {
    neverd::BinaryImage Image = makeCOFFImageRelativeRVASwitch(Options);
    neverd::Decoder Decoder;
    EXPECT_TRUE(Decoder.init(Image.Arch, Image.Mode));
    neverd::CFGBuilder Builder;
    const std::set<neverd::va_t> FunctionEntries{FunctionVA};
    Builder.setKnownFuncEntries(&FunctionEntries);
    return Builder.build(Image, Decoder, FunctionVA,
                         "coff_image_relative_rva_switch");
  };

  const neverd::LowFunc Exact = Recover();
  ASSERT_EQ(Exact.JumpTables.size(), 1u);
  const neverd::JumpTable &Table = Exact.JumpTables.front();
  EXPECT_EQ(Table.BaseAddr, TableVA);
  EXPECT_TRUE(Table.HasBaseAddr);
  EXPECT_EQ(Table.EntrySize, sizeof(uint32_t));
  EXPECT_TRUE(Table.IsRelative);
  EXPECT_FALSE(Table.IsSigned);
  EXPECT_TRUE(Table.HasTargetBase);
  EXPECT_EQ(Table.TargetBase, ImageBase);
  EXPECT_TRUE(Table.IsPEImageRelativeRVA);
  EXPECT_EQ(Table.Targets, ExpectedTargets);
  EXPECT_FALSE(lowFunctionHasOpcode(Exact, neverd::NdOp::INDIR_CALL));

  auto ExpectRejected = [&](COFFImageRelativeRVASwitchOptions Options,
                            const char *Reason) {
    SCOPED_TRACE(Reason);
    EXPECT_TRUE(Recover(Options).JumpTables.empty());
  };
  ExpectRejected({.ScalarStorageBase = true},
                 "scalar storage base cannot borrow address provenance");
  ExpectRejected({.ScalarTargetBase = true},
                 "scalar target base cannot borrow address provenance");
  ExpectRejected({.UseNonParameterIndex = true},
...
TEST_F(JTE_X86_64,
       TwoMappedLoadsWithoutRelocationIdentityRemainCallbackEvidence) {
  auto ImageOrErr = neverd::loadBinary(selectorOccurrenceX64Obj());
  ASSERT_TRUE(static_cast<bool>(ImageOrErr))
      << llvm::toString(ImageOrErr.takeError());
  const neverd::BinaryImage &Image = *ImageOrErr;
  const neverd::Symbol *Function =
      Image.findSymbol("jt_selector_twotable_raw_callback_control");
  const neverd::Symbol *Branch =
      Image.findSymbol("jt_selector_twotable_raw_callback_branch");
  const neverd::Symbol *Inner =
      Image.findSymbol("jt_selector_twotable_raw_inner");
  ASSERT_NE(Function, nullptr);
  ASSERT_NE(Branch, nullptr);
  ASSERT_NE(Inner, nullptr);
  ASSERT_EQ(Inner->Size, 16u);
  for (neverd::va_t Slot : {Inner->Addr, Inner->Addr + 8}) {
    EXPECT_FALSE(Image.CodePtrRelocSlots.count(Slot));
    EXPECT_FALSE(Image.RelCodeRelocSlots.count(Slot));
  }
  ...
  const neverd::LowFunc Exhausted = BuildWithBudget(0);
  EXPECT_TRUE(Exhausted.JumpTables.empty());
  EXPECT_TRUE(HasOpcodeAt(Exhausted, Branch->Addr, neverd::NdOp::INDIR_BR));
  EXPECT_FALSE(HasOpcodeAt(Exhausted, Branch->Addr, neverd::NdOp::INDIR_CALL));
  EXPECT_TRUE(Exhausted.UnsafeIndirectBranchAddresses.count(Branch->Addr));

  // This bounded account completes the no-relocation semantic rejection with
  // headroom.  Once that rejection is known, the earlier mapped loads must not
  // leave behind a composite-shape marker that suppresses callback lowering.
  const neverd::LowFunc Completed = BuildWithBudget(size_t{6} << 20);
  EXPECT_TRUE(Completed.JumpTables.empty());
  EXPECT_FALSE(HasOpcodeAt(Completed, Branch->Addr, neverd::NdOp::INDIR_BR));
  EXPECT_TRUE(HasOpcodeAt(Completed, Branch->Addr, neverd::NdOp::INDIR_CALL));
  EXPECT_FALSE(Completed.UnsafeIndirectBranchAddresses.count(Branch->Addr));
}

TEST_F(JTE_X86_64,
       ClampedTwoTableQuasiCopyLimitFailsClosedAfterPhysicalIdentity) {
  auto ImageOrErr = neverd::loadBinary(selectorOccurrenceX64Obj());
  ASSERT_TRUE(static_cast<bool>(ImageOrErr))
      << llvm::toString(ImageOrErr.takeError());
  const neverd::BinaryImage &Image = *ImageOrErr;
  const neverd::Symbol *Function =
      Image.findSymbol("jt_selector_clamped_depth_limit");
  const neverd::Symbol *Branch =
      Image.findSymbol("jt_selector_clamped_depth_branch");
  const neverd::Symbol *Inner =
      Image.findSymbol("jt_selector_clamped_depth_inner");
  ASSERT_NE(Function, nullptr);
  ASSERT_NE(Branch, nullptr);
  ASSERT_NE(Inner, nullptr);
  ASSERT_EQ(Inner->Size, 16u);
  ASSERT_TRUE(Image.CodePtrRelocSlots.count(Inner->Addr));
  ASSERT_TRUE(Image.CodePtrRelocSlots.count(Inner->Addr + 8));
  ...
  EXPECT_TRUE(HasOpcodeAt(neverd::NdOp::INDIR_BR));
  EXPECT_FALSE(HasOpcodeAt(neverd::NdOp::INDIR_CALL));
  EXPECT_TRUE(Low.UnsafeIndirectBranchAddresses.count(Branch->Addr));
  EXPECT_TRUE(Low.EverPublishedJumpTableBranchAddresses.empty());
}
...
TEST_F(JTE_X86_64, FreshBuilderRetainsPreviouslyPublishedOpaqueBranchIdentity) {
  auto ImageOrErr = neverd::loadBinary(lostPublishedX64Obj());
  ASSERT_TRUE(static_cast<bool>(ImageOrErr))
      << llvm::toString(ImageOrErr.takeError());
  neverd::BinaryImage &Image = *ImageOrErr;
  ...
  const neverd::LowFunc First =
      FirstBuilder.build(Image, FirstDecoder, Function->Addr, Function->Name);
  ...
  ASSERT_NE(
      std::find(First.JumpTables.front().SuppressibleRelocationSlots.begin(),
  ...
  const neverd::LowFunc NoSeed =
      NoSeedBuilder.build(Image, NoSeedDecoder, Function->Addr, Function->Name);
  ...
  const neverd::LowFunc Seeded =
      SeededBuilder.build(Image, SeededDecoder, Function->Addr, Function->Name);
  ...
  const neverd::LowFunc Reused =
      SeededBuilder.build(Image, ReusedDecoder, Function->Addr, Function->Name);
  ...
  EXPECT_EQ(PipelineLow->EverPublishedJumpTableBranchAddresses.count(Branch),
            1u);
...
TEST_F(JTE_X86_64, RelocationRunLimitRequiresCompleteIdentityScan) {
  ...
  auto CountSlots = [](const std::set<neverd::va_t> &Slots, neverd::va_t Base,
  ...
  EXPECT_EQ(
      neverd::countCodePtrRelocRun(Image, ExactTable->Addr, 8, &ExactComplete),
      Limit);
  ...
  for (llvm::StringRef Name : {"jt_identity_over_limit_absolute_callback",
    ...
    EXPECT_FALSE(lowFunctionHasOpcode(Overflow, neverd::NdOp::INDIR_CALL))
        << Name.str();
    EXPECT_FALSE(Overflow.UnsafeIndirectBranchAddresses.empty()) << Name.str();
  }
  ...
  auto AbsoluteSymbol =
      std::find_if(SizedImage.Symbols.begin(), SizedImage.Symbols.end(),
                   [&](const neverd::Symbol &S) {
                     return S.Name == "jt_identity_over_limit_absolute_table";
                   });
...
TEST_F(JTE_X86_64, ModuloBoundAcceptsExactFactorizedSixBackMultiply) {
  ...
  ASSERT_TRUE(static_cast<bool>(ImageOrErr))
      << llvm::toString(ImageOrErr.takeError());
  const neverd::BinaryImage &Image = *ImageOrErr;
  const neverd::Symbol *Function = Image.findSymbol("jt_modulo_factorized_six");
  ASSERT_NE(Function, nullptr);
...
TEST_F(JTE_X86_64, ModuloBoundChecksMaskedBackMultiply) {
  auto ImageOrErr = neverd::loadBinary(moduloDomainObj());
  ASSERT_TRUE(static_cast<bool>(ImageOrErr))
      << llvm::toString(ImageOrErr.takeError());
  const neverd::BinaryImage &Image = *ImageOrErr;
  for (const auto &[Name, Count] : {std::pair{"jt_modulo_masked_six", 6u},
                                    {"jt_modulo_masked_twelve", 12u},
                                    {"jt_modulo_masked_product", 6u},
                                    {"jt_modulo_masked_wrong_mask", 0u},
                                    {"jt_modulo_masked_wrong_magic", 0u},
                                    {"jt_modulo_masked_wrong_shift", 0u},
                                    {"jt_modulo_masked_wrong_factor", 0u}}) {
    SCOPED_TRACE(Name);
    const auto *Function = Image.findSymbol(Name);
    ASSERT_NE(Function, nullptr);
    const auto *Table = Image.findSymbol(std::string(Name) + "_table");
    ASSERT_NE(Table, nullptr);
    const unsigned PhysicalCount = Count == 12 ? 12 : 6;
    ASSERT_EQ(Table->Size, PhysicalCount * 8u);
    for (unsigned I = 0; I < PhysicalCount; ++I)
      ASSERT_EQ(Image.CodePtrRelocSlots.count(Table->Addr + I * 8), 1u);
    neverd::Decoder Decoder;
    ASSERT_TRUE(Decoder.init(Image.Arch, Image.Mode));
    neverd::CFGBuilder Builder;
    const auto Low = Builder.build(Image, Decoder, Function->Addr, Name);
    if (Count == 0) {
      ...
      if (!Low.JumpTables.empty())
        EXPECT_EQ(Low.JumpTables.front().Targets.size(), Count);
      EXPECT_TRUE(Low.UnsafeIndirectBranchAddresses.empty());
    }
  }
}
...
TEST_F(JTE_X86_64, ModuloBoundDecodesTargetsBeforeFixedPointPublication) {
  ...
  neverd::BinaryImage &Image = *ImageOrErr;
  const neverd::Symbol *Function =
      Image.findSymbol("jt_modulo_lfp_rejects_undecoded_late_escape");
  const neverd::Symbol *Table =
  ...
  const size_t AbsoluteCodePointers = std::count_if(
      Image.CodePtrRelocSlots.begin(), Image.CodePtrRelocSlots.end(),
      [&](neverd::va_t Slot) {
  ...
  const size_t RelativeCodeSlots = std::count_if(
      Image.RelCodeRelocSlots.begin(), Image.RelCodeRelocSlots.end(),
      [&](neverd::va_t Slot) {
        return Slot >= Table->Addr && Slot < Table->Addr + Table->Size;
      });
...
TEST_F(JTE_X86_64,
  ...
  const neverd::LowFunc Exhausted =
      BuildWithBudget(MinimumSuccessfulBudget - 1, &HasPendingExploration);
  EXPECT_TRUE(Exhausted.JumpTables.empty());
  EXPECT_TRUE(lowFunctionHasOpcode(Exhausted, neverd::NdOp::INDIR_BR));
  EXPECT_FALSE(lowFunctionHasOpcode(Exhausted, neverd::NdOp::INDIR_CALL));
...
TEST_F(JTE_X86_64, ModuloBoundSharedDAGStaysWithinEvidenceBudget) {
  ...
  const neverd::Symbol *Table = Image.findSymbol("jt_modulo_shared_dag_table");
  ASSERT_NE(Function, nullptr);
  ASSERT_NE(Table, nullptr);
  ASSERT_EQ(Table->Size, 5u * 8u);
...
TEST_F(JTE_X86_64, DeadMaskDependencyDoesNotOverrideModuloDomain) {
  ...
  for (const neverd::LowBlock &Block : Low.Blocks)
    for (const neverd::LowOp &Op : Block.Ops) {
      ...
      SawMultiplyByZero |= Op.Opcode == neverd::NdOp::INT_MULT && HasScalar(0);
...
TEST_F(JTE_X86_64, MaskBoundIntersectsTranslatedNestedMergeDomains) {
  ...
  ASSERT_EQ(Low.JumpTables.size(), 1u);
  ASSERT_EQ(Low.JumpTables.front().Targets.size(), 2u);
  EXPECT_EQ(Low.JumpTables.front().CaseLabels, (std::vector<int64_t>{1, 2}));
}
...
TEST_F(JTE_X86_64, MaskFixedPointRejectsLateOutOfDomainSelector) {
  ...
  ASSERT_TRUE(SawMask7) << "case zero must expose the eight-value mask domain";
...
TEST_F(JTE_X86_64, MaskFixedPointReplaysCompleteEntryDomain) {
  ...
  const neverd::Symbol *Table =
      Image.findSymbol("jt_identity_mask_fp_entry_bound_late_escape_table");
  ASSERT_NE(Function, nullptr);
  ASSERT_NE(Table, nullptr);
  ASSERT_EQ(Table->Size, 8u * 8u);
  EXPECT_EQ(std::count_if(Image.CodePtrRelocSlots.begin(),
...
TEST_F(JTE_X86_64,
  ...
  auto CheckLayout = [&](const char *FunctionName, const char *TableName,
    ...
    auto FindIn = [&](const neverd::Symbol &Begin,
                      const neverd::Symbol &End) -> const neverd::JumpTable * {
    ...
    for (size_t I = 0; I < AddressOrder.size(); ++I) {
      ...
      for (const neverd::Symbol *Target : Targets)
        EXPECT_EQ(
            std::count(JT->Targets.begin(), JT->Targets.end(), Target->Addr), 1)
      ...
      EXPECT_NE(findExactLowOp(Low, Load.Addr, Load.Seq, neverd::NdOp::LOAD),
                nullptr);
  ...
  CheckLayout(
      ...
      {"jt_identity_sibling_nf_t0_begin", "jt_identity_sibling_nf_t1_begin",
       "jt_identity_sibling_nf_t2_begin", "jt_identity_sibling_nf_t3_begin"},
  ...
  CheckLayout(
      "jt_identity_sibling_snapshot_wide_first", "jt_identity_sibling_wf_table",
      {"jt_identity_sibling_wf_t0_begin", "jt_identity_sibling_wf_t1_begin",
       "jt_identity_sibling_wf_t2_begin", "jt_identity_sibling_wf_t3_begin"},
...
TEST_F(JTE_X86_64, IncompleteMaskDomainDoesNotCaptureIndexedCallbackTailCall) {
  auto ImageOrErr = neverd::loadBinary(maskEqualBoundObj());
  ASSERT_TRUE(static_cast<bool>(ImageOrErr))
      << llvm::toString(ImageOrErr.takeError());
  neverd::BinaryImage &Image = *ImageOrErr;
...
TEST_F(JTE_X86_64, IncompleteMaskDomainValidatesWholeSizedCallbackObject) {
  auto ImageOrErr = neverd::loadBinary(maskEqualBoundObj());
  ASSERT_TRUE(static_cast<bool>(ImageOrErr))
      << llvm::toString(ImageOrErr.takeError());
  neverd::BinaryImage &Image = *ImageOrErr;
  const neverd::Symbol *Function =
      Image.findSymbol("jt_identity_incomplete_mask_prefix_callback_tailcall");
  const neverd::Symbol *Table =
      Image.findSymbol("jt_identity_incomplete_mask_prefix_callback_table");
  ASSERT_NE(Function, nullptr);
  ASSERT_NE(Table, nullptr);
  ASSERT_EQ(Table->Size, 4u * 8u);
  EXPECT_EQ(std::count_if(Image.CodePtrRelocSlots.begin(),
...
TEST_F(JTE_X86_64,
  ...
  neverd::BinaryImage &Image = *ImageOrErr;
  const neverd::Symbol *Function =
      Image.findSymbol("jt_identity_incomplete_mask_nested_function_tailcall");
  const neverd::Symbol *Table =
      Image.findSymbol("jt_identity_incomplete_mask_nested_function_table");
...
TEST_F(JTE_X86_64, StackMemcpyLengthClobberFailsClosed) {
  ...
  ASSERT_TRUE(static_cast<bool>(ImageOrErr))
      << llvm::toString(ImageOrErr.takeError());
  neverd::LowFunc Low =
      buildNamedLowFunction(*ImageOrErr, "jt_stack_memcpy_length_clobber");
...
TEST_F(JTE_X86_64, StackTableEvidenceBudgetExhaustionIsTransactionalAndOpaque) {
...
static neverd::BinaryImage
a64StrictSourceSpanImage(bool RightWritable = false) {
  neverd::BinaryImage Image;
...
TEST_F(JTE_AArch64, RelocationFreeLoadCannotCrossSourceOwner) {
  ...
  EXPECT_FALSE(neverd::exactImmutableDataSpanOwner(Image, 0x100c, 8, 0x1000))
...
TEST_F(JTE_AArch64, RelocationFreeLoadCannotDereferenceOwnerOnePast) {
  ...
  EXPECT_FALSE(neverd::exactImmutableDataSpanOwner(Image, 0x1010, 8, 0x1000))
...
TEST_F(JTE_AArch64, RelocationFreeMemcpyRejectsRuntimeWritableSourceSpan) {
  ...
  ASSERT_EQ(neverd::exactImmutableDataSpanOwner(Image, 0x1000, 0x10, 0x1000),
            std::optional<neverd::va_t>{0x1000});
  EXPECT_FALSE(neverd::exactImmutableDataSpanOwner(Image, 0x1010, 0x10, 0x1010))
...
TEST_F(JTE_AArch64, MaskBoundRejectsConstantSelectorWithUnreachableProducer) {
  ...
  const neverd::Symbol *Table = Image.findSymbol("a64_constant_selector_table");
  ASSERT_NE(Function, nullptr);
  ASSERT_NE(Table, nullptr);
  ASSERT_EQ(Table->Size, 4u * 8u);
  EXPECT_EQ(std::count_if(Image.CodePtrRelocSlots.begin(),
...
TEST_F(JTE_AArch64, ModuloBoundRejectsDestructiveDivisionNumerator) {
  ...
  const neverd::Symbol *Table = Image.findSymbol("a64_destructive_udiv_table");
  ASSERT_NE(Function, nullptr);
  ASSERT_NE(Table, nullptr);
  ASSERT_EQ(Table->Size, 5u * 8u);
  EXPECT_EQ(std::count_if(Image.CodePtrRelocSlots.begin(),
...
TEST_F(JTE_AArch64, ModuloBoundReplaysIneligibleConditionalSignedOldTargets) {
  auto ImageOrErr = neverd::loadBinary(indexIdentityA64Obj());
  ASSERT_TRUE(static_cast<bool>(ImageOrErr))
      << llvm::toString(ImageOrErr.takeError());
  neverd::BinaryImage &Image = *ImageOrErr;
...
TEST_F(JTE_AArch64, CompactIndexAcceptsExplicitWToXZeroExtension) {
  ...
  auto Run = runPipelineWithEvidenceBudget(
      *ImageOrErr, neverd::limits::kMaxJumpTableEvidenceWork);
  ASSERT_TRUE(Run.Result.Success) << Run.Result.Error;
  ...
  EXPECT_TRUE(std::all_of(Function->JumpTables.front().SelectorUseRefs.begin(),
...
TEST_F(JTE_X86_64, MaskKnownOneWitnessDoesNotLeakFromUnrelatedSiblingPath) {
  auto ImageOrErr = neverd::loadBinary(identityCfgLaneObj());
  ASSERT_TRUE(static_cast<bool>(ImageOrErr))
      << llvm::toString(ImageOrErr.takeError());
  neverd::BinaryImage &Image = *ImageOrErr;
...
TEST_F(JTE_X86_64, MaskKnownOneFixedPointRejectsSelfBootstrappedProducer) {
  auto ImageOrErr = neverd::loadBinary(identityCfgLaneObj());
  ASSERT_TRUE(static_cast<bool>(ImageOrErr))
      << llvm::toString(ImageOrErr.takeError());
  neverd::BinaryImage &Image = *ImageOrErr;
