...
La revisione di pacchetto predefinita è `neverd-llvm-v23.0.0-r3`. Le directory di compilazione che conservano il vecchio tag senza revisione, `neverd-llvm-v23.0.0-r1` o `neverd-llvm-v23.0.0-r2` passano automaticamente a `r3`, salvo quando `NEVERD_LLVM_PREBUILT_SHA256` è specificato esplicitamente; in quel caso il tag originale viene mantenuto.

Il tag di release versiona il pacchetto NeverD, mentre `BUILDINFO.txt` registra l'esatto commit del fork LLVM. Se LLVM continua a riportare `23.0.0` ma i sorgenti del fork sono cambiati, la scelta immutabile consueta è una revisione di pacchetto come `neverd-llvm-v23.0.0-r4` (poi `-r5`), non `23.0.1`, a meno che non sia cambiata la patch version di LLVM stesso. Puntare `NEVERD_LLVM_PREBUILT_TAG` a quella nuova revisione.
