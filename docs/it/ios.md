...
Il catalogo C ABI di Swift è generato da dichiarazioni upstream a revisione fissata. Accetta solo firme fisse incondizionate del modulo Swift con convenzione C ordinaria, puntatori noti e lunghezze senza segno della larghezza di un puntatore. Interi sconosciuti, ritorni multipli, convenzioni Swift speciali e alternative in conflitto restano esclusi. Le chiamate conservano gli effetti di allocazione, inizializzazione e distruzione e i tipi specializzati dei contratti esistenti. Rigenerare con `scripts/generate_swift_c_declarations.py --input RuntimeFunctions.def --source-sha256 <pinned-hash>`; aggiungere `--check` verifica il catalogo. Revisione, hash e avvisi di terze parti accompagnano i dati.
