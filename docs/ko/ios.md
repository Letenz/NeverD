...
로컬 네이티브 보조 함수는 보존 레지스터의 읽기 전용 정수 컨텍스트도 명시적 인수로 복원할 수 있습니다. 기존 진입 값 관찰 분석이 실제 사용을 확인하고, 완전한 LowIR이 전체 너비 읽기와 프레임 이외 보존 레지스터에 대한 쓰기 부재를 증명해야 합니다. 재생성한 호출자와 함수 정의에는 동일한 물리 인수를 바인딩합니다. 외부 C 또는 Swift 원형을 추측하지 않으며, 숨겨진 레지스터 출력, 불충분한 증명, 불완전한 종속성은 지원하지 않습니다.
