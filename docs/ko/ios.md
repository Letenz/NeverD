...
제한된 LowIR 분석이 모든 종료 경로에서 보존 레지스터 바이트, 스택 포인터 및 링크 레지스터의 복원을 증명하면 void 네이티브 요약은 스택 프레임과 일반 호출도 지원합니다. 부분 쓰기, 암시적 0 확장, 겹치는 저장 및 호출로 인한 변경은 관련 증거를 무효화하며 프레임 주소의 스필이나 외부 전달은 증명을 거부합니다. 다시 리프팅한 뒤 HighIR에 전혀 나타나지 않는 보조 레지스터 인수를 제거하고 파이프라인을 다시 실행할 수 있습니다. 비공개 저장 제거는 기존 HighIR 정리가 담당하며 표준 인수와 실제 입력 사용은 유지합니다.
