...
Swift C ABI 목록은 고정된 상위 저장소 선언에서 생성합니다. Swift 모듈의 무조건 고정 시그니처 중 일반 C 호출 규약, 알려진 포인터 표현, 포인터 폭의 부호 없는 길이만 허용합니다. 알 수 없는 정수, 다중 반환, 특수 Swift 규약, 충돌 선언은 제외합니다. 호출의 할당·초기화·소멸 효과와 기존 계약의 특수 타입을 유지합니다. `scripts/generate_swift_c_declarations.py --input RuntimeFunctions.def --source-sha256 <pinned-hash>`로 다시 생성하고 `--check`로 저장된 목록을 검증합니다. 리비전, 내용 해시, 제삼자 고지가 생성 결과에 포함됩니다.
