...
검증된 외부 void 꼬리 호출로 전달하는 리프 보조 함수는 완전한 스칼라 결과가 없을 때 내부 void 소스 서명을 사용할 수 있습니다. LowIR 의 모든 호출과 합성 반환이 일치해야 하며 보존 레지스터나 스택/프레임 포인터를 쓰면 안 됩니다. CFG, 본문과 의존성 검사는 유지됩니다. 결과 비트는 제공하지 않으므로 알 수 없는 결과를 읽는 호출자는 미복원 상태로 남습니다. 실행 회귀는 조건부 객체 소멸, 독립적인 호출자 반환값과 알 수 없는 결과를 읽는 호출자의 거부를 확인합니다.
