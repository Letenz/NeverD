...
Le catalogue C ABI de Swift est généré depuis des déclarations amont à révision fixe. Il accepte uniquement les signatures fixes inconditionnelles du module Swift en convention C ordinaire, avec des pointeurs connus et des longueurs non signées de largeur pointeur. Les entiers inconnus, retours multiples, conventions Swift particulières et alternatives conflictuelles restent exclus. Les appels conservent les effets d’allocation, d’initialisation et de destruction, ainsi que les types spécialisés des contrats existants. Régénérer avec `scripts/generate_swift_c_declarations.py --input RuntimeFunctions.def --source-sha256 <pinned-hash>` ; ajouter `--check` vérifie le catalogue. Révision, empreinte et notices tierces accompagnent les faits.
