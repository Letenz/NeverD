...
Les fonctions natives locales peuvent aussi exposer des contextes entiers en lecture seule dans des registres préservés. L’analyse existante des valeurs observables à l’entrée doit établir leur utilisation, et le LowIR complet doit prouver une lecture de largeur entière sans écriture dans les registres préservés hors cadre. Les appelants et définitions régénérés utilisent les mêmes paramètres physiques explicites. Cela ne déduit aucun prototype C ou Swift externe ; les sorties cachées de registres, les preuves indisponibles et les dépendances incomplètes restent non prises en charge.
