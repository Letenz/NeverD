...
Les déclarations de fonctions conservent la convention d’appel dans l’identité de leur signature. La couche ABI commune prend en charge les appels Swift limités aux paramètres entiers ou pointeurs de 64 bits passés en registres et aux résultats de deux mots entiers ou pointeurs au maximum ; HighC conserve `swiftcall` dans les déclarations et définitions. Les imports de métadonnées Foundation publiques exigent un accord entre graphes de symboles du compilateur, IR des requêtes réelles et exports SDK exacts sur ARM64/x86-64 pour macOS et Mac Catalyst. Un suffixe de symbole ne suffit pas à établir l’ABI. Les arguments génériques ou cachés, les types de rappel Swift et les supports physiques non pris en charge restent rejetés. Les déclarations Mac Catalyst ne prouvent pas une validation sur appareil iOS.
