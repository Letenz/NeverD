...
本地原生輔助函式也可將保持型暫存器中的唯讀整數上下文恢復為明確參數。既有入口可觀察性分析必須確認該值被使用，完整 LowIR 必須證明存在完整寬度的原生讀取，且不會寫入框架暫存器之外的保持型暫存器。重建的呼叫者與函式定義使用相同的實體參數繫結。這不推斷外部 C 或 Swift 原型；隱藏的暫存器輸出、不可用的證明及不完整的相依鏈仍不支援。
