...
Objective-C 匯出也支援一組固定、使用一般 C ABI 的 Swift 執行階段匯入：參照計數、原生及未知物件弱參照、物件中繼資料，以及存取檢查的開始與結束。產生的 C 保留這些呼叫，連結時需要 Swift 執行階段。專用暫存器入口、Swift 呼叫慣例及任意 Swift 符號仍不支援；匯入身分與純量載體必須精確符合。
