...
検証済みの外部 void 末尾呼び出しへ転送するリーフ補助関数は、完全なスカラー結果がない場合、内部 void ソース署名を使用できます。LowIR の全呼び出しと合成 return の一致、および保存レジスターやスタック／フレームポインターへの書き込みがないことが必要です。CFG、本体、依存関係の検証は継続します。結果ビットは供給されず、未知の結果を読む呼び出し元は未復元のままです。実行回帰では条件付きオブジェクト破棄、独立した呼び出し元の戻り値、未知の結果を読む呼び出し元の拒否を確認します。
