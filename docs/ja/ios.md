...
void のネイティブ要約は、制限付き LowIR 解析がすべての出口で保存対象のレジスタバイト、スタックポインタ、リンクレジスタの復元を証明できる場合、スタックフレームと通常の呼び出しにも対応します。部分書き込み、暗黙のゼロ拡張、重複ストア、呼び出しによる破壊は対応する事実を無効にし、フレームアドレスのスピルやエスケープは証明を拒否します。再リフト後、HighIR に一度も現れない補助レジスタ引数を除去してパイプラインを再実行できます。非公開ストアの除去は既存の HighIR クリーンアップが担い、通常の引数と実際の入力使用は保持します。
