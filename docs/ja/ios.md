...
対応する Objective-C Block 呼び出しには、暗黙の Block オブジェクトと全引数・戻り値の格納位置を含む、完全な固定スカラー呼び出し ABI が必要です。実行時エンコーディング `@?` を `id` に広げるのは宣言だけであり、呼び出しプロトタイプは確定しません。グローバル Block の参照は共有オブジェクトの同一性を維持します。対応する同期スカラーキャプチャでは、実際の格納位置と呼び出しフローを証明します。検証済みの `objc_retainBlock` / `_Block_copy` 呼び出しでコピーされる強参照キャプチャは、直線的な構築で格納領域が初期化され、呼び出し・コピー・破棄の関数本体が完全に復元された場合にエスケープできます。生成した記述子は元のヘルパー ABI と所有権レイアウトを維持します。弱参照/byref の所有権、未知のレイアウト、未検証の利用先は未復元のままです。
...
コンパイラが宣言した NSString の書式呼び出しでは、検証済みの定数書式オブジェクトから昇格後のスカラー引数を復元できます。順序指定と位置指定は、欠落や型の矛盾がない場合に限ります。Darwin arm64 は可変引数を 8 バイトのスタックスロットから読み、x86_64 は整数・浮動小数点レジスタとスタックを使います。生成呼び出しは省略記号と動的ディスパッチを保持します。未知の書式、文字数の書き込み、long double、未対応の拡張は拒否します。
...
システムの Foundation をインポートするリンク済み 64 ビット Darwin イメージでは、コンパイラから抽出した組み込みのフレームワーク宣言も参照します。同じセレクタのランタイム宣言とフレームワーク宣言はすべて互換である必要があり、可変引数、未対応の集約型、プラットフォーム間で不一致のシグネチャは未解決のままです。このカタログは呼び出し型だけを提供し、レシーバのクラスや関数本体は復元しません。NeverD の使用にローカルの Apple SDK は不要です。
...
外部データの束縛には、各プラットフォームで共通の非 TLS SDK 宣言と、正確なライブラリエクスポートの証拠が必要です。生成 C は実際のシンボルの記憶域を参照し、グローバルポインタと参照先の区別を含め、後続のメモリアクセスを保持します。弱いインポート、識別の競合、未対応の記憶域は未束縛のままです。データ宣言だけでは block の構築や所有権を証明できません。
