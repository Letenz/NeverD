...
Swift の C ABI カタログは固定リビジョンの上流宣言から生成します。Swift モジュールの無条件で固定されたシグネチャのうち、通常の C 呼び出し規約、既知のポインタ表現、ポインタ幅の符号なし長さだけを受け付けます。不明な整数型、複数の戻り値、特殊な Swift 規約、競合する宣言は対象外です。割り当て、初期化、破棄の効果と既存契約の専用型を維持します。`scripts/generate_swift_c_declarations.py --input RuntimeFunctions.def --source-sha256 <pinned-hash>` で再生成し、`--check` を付けて登録済みカタログを検証します。リビジョン、内容ハッシュ、第三者の通知を生成結果に付記します。
