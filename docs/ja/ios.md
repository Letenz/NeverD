...
既知の Objective-C ランタイムのインポート呼び出しには、retain/release、自動解放、強参照・弱参照の保存、オブジェクト割り当て、固定シグネチャのプロパティ設定について、明示的な引数と戻り値の対応があります。ARM64 のレジスタ専用 retain/release は指定レジスタを読み取ります。インポートの識別と ABI の一致が必要です。関連オブジェクトの取得・設定・削除では、オブジェクト、キー、値、ポインタ幅のポリシー引数を保持し、生成 C は公開ランタイムヘッダーを使います。macOS の再コンパイル試験で寿命、弱参照のゼロ化、コピー、削除を元のメソッドと比較します。キーにも再配置可能なソースへの対応が必要で、未解決のイメージアドレスは完全な復元を妨げます。
