...
ローカルのネイティブ補助関数では、保存対象レジスタにある読み取り専用の整数コンテキストも明示的な引数にできます。既存の入口可観測性解析が値の使用を確認し、完全な LowIR が全幅の読み取りと、フレーム以外の保存対象レジスタへの書き込みがないことを証明する必要があります。再生成する呼び出し元と関数定義には同じ物理引数を束縛します。外部 C または Swift のプロトタイプを推測するものではなく、隠れたレジスタ出力、証明不足、不完全な依存関係は未対応です。
...
外部データの束縛には、各プラットフォームで共通の非 TLS SDK 宣言と、正確なライブラリエクスポートの証拠が必要です。生成 C は実際のシンボルの記憶域を参照し、グローバルポインタと参照先の区別を含め、後続のメモリアクセスを保持します。弱いインポート、識別の競合、未対応の記憶域は未束縛のままです。データ宣言だけでは block の構築や所有権を証明できません。 カタログには CoreData、CoreGraphics、ImageIO、CoreSpotlight のデータを含みます。組み込みリテラルの記憶域は、各ターゲットで空コレクションと真偽値オブジェクトをコンパイルして取得します。外部の非 TLS データへの直接アドレスだけを認め、同じエクスポート検証を適用します。カタログ生成には libclang に加えて `--clang` で指定する Clang コンパイラが必要です。
...
Swift ランタイム ABI カタログは固定リビジョンの上流宣言から生成し、C または Swift の呼び出し規約を保持します。既知のポインタとポインタ幅の符号なし型による固定スカラー署名を扱います。Swift 呼び出しには正確な非弱 libswiftCore インポートが必要で、対応する二種類のバージョン付きメタデータ可用性も弱インポートを許可しません。特殊引数レジスタ、不明な表現、複数結果、未対応の可用性、競合宣言は除外します。swift_willThrow は宣言 DSL にない swiftself/swifterror 属性をコンパイラが追加するため対象外です。副作用と既存のコールバック・バイト契約を保持します。`scripts/generate_swift_runtime_declarations.py --input RuntimeFunctions.def --source-sha256 <pinned-hash>` で再生成し、`--check` で検証します。リビジョン、ハッシュ、第三者通知を付記します。
