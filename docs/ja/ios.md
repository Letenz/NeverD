...
不変バイトバッファーは、検証済みのインポート呼び出し契約が非負の読み取り長を制限し、書き込み、ポインターの保持、アドレス同一性の比較を除外する場合にのみ再構築されます。元のバイトと長さを保持し、その他のポインター用途は未解決のままです。Swift の診断報告では宣言された C ABI と正確なリンカーシンボルを使い、その後の終了トラップを独立した操作として保持します。
