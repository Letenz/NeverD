...
Экспериментальная команда `neverd mobile app.apk -o recovered-app` восстанавливает Java из APK, DEX и smali и создаёт `report.json`. Движок по умолчанию реализован на C++20; для его работы не нужны Python, Java или JADX. Ни `NEVERD_JADX`, ни исполняемый файл `jadx` в PATH не выбирают внешний движок: для этого нужен явный `--jadx PATH`. Автоматического перехода на другой движок нет. Пути с пробелами заключайте в кавычки. Поддерживаемые входы, отчёты и ограничения описаны в [руководстве Android](android.md).
...
### Восстановление исходников Android / iOS
...
Объём восстановления зависит от поддерживаемых шаблонов кода. Ограничения описаны в соответствующих руководствах.
...
Ревизия пакета по умолчанию — `neverd-llvm-v23.0.0-r3`. Существующие каталоги сборки, сохраняющие старый тег без номера ревизии, `neverd-llvm-v23.0.0-r1` или `neverd-llvm-v23.0.0-r2`, автоматически переходят на `r3`. Если `NEVERD_LLVM_PREBUILT_SHA256` задан явно, исходный тег сохраняется.

Тег релиза версионирует пакет NeverD, а `BUILDINFO.txt` фиксирует точный commit форка LLVM. Если LLVM по-прежнему сообщает `23.0.0`, но исходники форка изменились, обычный неизменяемый выбор — ревизия пакета вроде `neverd-llvm-v23.0.0-r4` (затем `-r5`), а не `23.0.1`, если только не изменилась собственная patch-версия LLVM. Направьте `NEVERD_LLVM_PREBUILT_TAG` на эту новую ревизию.
...
[примеры API C для EVM](evm.md#api-c).
...
При распространении копий или переработок кода NeverD, на который распространяется AGPL, необходимо сохранять уведомления об авторских правах и лицензии в соответствии с AGPL; это также относится к повторному использованию с помощью ИИ/LLM и преобразованиям с помощью LLVM. Чтобы можно было проследить происхождение кода, рекомендуем также указывать репозиторий, файлы и использованную версию. См. [руководство по указанию авторства и источников](ATTRIBUTION.md), [NOTICE](../../NOTICE) и [CITATION.cff](../../CITATION.cff).
