...
源码记录类型独立保存字段布局。统一 ABI 层支持 Darwin ARM64 中含一至四个同类 float 或 double 成员的嵌套结构体，并在浮点寄存器不足时将整个参数放到栈上。MedIR 在 SSA 前绑定每个物理成员，HighIR 保留单个逻辑参数或返回值，C 输出校验布局。混合类型、其他架构及不完整的分量列表仍明确拒绝；这些提示不能授权二进制改写。
