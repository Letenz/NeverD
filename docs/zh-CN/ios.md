...
本地原生辅助函数还可将保持型寄存器中的只读整数上下文恢复为显式参数。现有入口可观察性分析必须确认该值被使用，完整 LowIR 必须证明存在完整宽度的原生读取，且不会写入帧寄存器之外的保持型寄存器。重建的调用者与函数定义使用相同的物理参数绑定。这不推断外部 C 或 Swift 原型；隐藏的寄存器输出、不可用的证明及不完整的依赖链仍不支持。
