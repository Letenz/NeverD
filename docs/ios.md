...
Local native helpers can also expose read-only integer context inputs held in preserved registers. The existing observable-entry analysis must require the value, and complete LowIR must show a full-width native read without writes to preserved non-frame registers. The same explicit physical parameters bind the regenerated callers and definitions. This does not infer an external C or Swift prototype; hidden register outputs, unavailable proofs and incomplete dependency closures remain unsupported.
