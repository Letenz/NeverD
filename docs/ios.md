...
The Swift runtime C ABI catalog is generated from a pinned upstream declaration file. It accepts only unconditional fixed signatures in the Swift module using the ordinary C convention, known pointer representations, and pointer-sized unsigned lengths. Unknown integers, multiple return values, special Swift conventions and conflicting alternatives remain unsupported. Runtime calls retain allocation, initialization and destruction effects; existing callback and bounded-byte contracts keep their specialized types. Regenerate with `scripts/generate_swift_c_declarations.py --input RuntimeFunctions.def --source-sha256 <pinned-hash>`, adding `--check` to verify the checked-in catalog. The revision, content hash and third-party notices accompany the generated facts.
