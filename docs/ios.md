...
Associated-object get/set/remove calls preserve the object, key, value, and pointer-sized policy arguments. Generated C uses the public Objective-C runtime header, and executable regression tests compare retention, copying, and removal with the original methods. Association keys still need a relocatable source binding; unresolved image addresses remain a recovery limitation.
