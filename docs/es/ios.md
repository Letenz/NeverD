...
El catálogo C ABI de Swift se genera desde declaraciones upstream de una revisión fija. Solo acepta firmas fijas incondicionales del módulo Swift con convención C ordinaria, punteros conocidos y longitudes sin signo del ancho de un puntero. Enteros desconocidos, retornos múltiples, convenciones Swift especiales y alternativas conflictivas siguen excluidos. Se conservan los efectos de asignación, inicialización y destrucción y los tipos especializados de contratos existentes. Regenerar con `scripts/generate_swift_c_declarations.py --input RuntimeFunctions.def --source-sha256 <pinned-hash>`; añadir `--check` verifica el catálogo. Los hechos incluyen revisión, hash y avisos de terceros.
