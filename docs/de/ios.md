...
Eine separate Bindung unterstützt auf arm64 und x86_64 den exakten Darwin-Import String → NSString `_$sSS10FoundationE19_bridgeToObjectiveCSo8NSStringCyF`. Sie erhält beide String-Wörter, das zurückgegebene Objekt mit Eigentumsübertragung und Clang `swiftcall`; zum Linken sind Swift Foundation und Swift Core erforderlich. Rückwärts-Bridging und andere Swift-Signaturen bleiben ununterstützt.
...
Numerische Profilzähler in einem vollständig begrenzten, zeigerfreien Abschnitt `__DATA,__llvm_prf_cnts` können gemeinsamen rekonstruierten Speicher verwenden. Anfangsbytes, überlappende Lese- und Schreibzugriffe von 1–16 Byte sowie Änderungen bleiben über Methodendateien hinweg erhalten. Der mobile Export führt die Hilfsfunktionen zusammen; Nutzer der C-API müssen jede Definition aus `shared_storage_functions` genau einmal linken. Dieser Speicher ist unabhängig vom Originalabbild und dessen Profiling-Laufzeit. Entweichende Adressen, geordnete Zugriffe, unvollständige Abbildungen und Zeigerrelokationen bleiben ununterstützt.
