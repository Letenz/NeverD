...
Der Swift-C-ABI-Katalog wird aus einer festgelegten Upstream-Deklarationsdatei erzeugt. Er akzeptiert nur unbedingte feste Signaturen im Swift-Modul mit gewöhnlicher C-Aufrufkonvention, bekannten Zeigertypen und vorzeichenlosen Längen in Zeigerbreite. Unbekannte Ganzzahlen, mehrere Rückgabewerte, besondere Swift-Konventionen und widersprüchliche Alternativen bleiben ausgeschlossen. Laufzeitaufrufe behalten Allokations-, Initialisierungs- und Destruktionseffekte; bestehende Callback- und Byteverträge behalten ihre speziellen Typen. Erzeugen mit `scripts/generate_swift_c_declarations.py --input RuntimeFunctions.def --source-sha256 <pinned-hash>`; `--check` prüft den eingecheckten Katalog. Revision, Inhalts-Hash und Fremdhinweise begleiten die Fakten.
