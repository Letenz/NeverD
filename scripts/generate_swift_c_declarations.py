#!/usr/bin/env python3
"""Extract fixed pointer/size C ABI facts from pinned Swift runtime declarations.

Only unconditional, directly written FUNCTION records are inputs. Macro-generated
families, unknown representations and other calling conventions supply no facts.
...
def declarations(source):
    ...
    for match in re.finditer(r'\bFUNCTION\s*\(', text):
        ...
        fact = None
        if (module == 'Swift' and convention == 'C_CC' and
                availability == 'AlwaysAvailable' and result and len(result) == 1 and
                args is not None and len(args) <= 16 and 'v' not in args and
                (attributes or attrs == 'NO_ATTRS') and
                (not no_return or result == ['v'])):
            fact = ''.join(result + args), no_return
...
def render(facts, digest):
    lines = ['// clang-format off',
             '// Generated by scripts/generate_swift_c_declarations.py.',
             ...
             '// Changes: extract fixed pointer/size C ABI declarations, 2026-09-15.',
             '// Implementations and runtime effect assumptions are not included.',
             'static constexpr SwiftCDeclaration SwiftCDeclarations[] = {']
    for name, (signature, no_return) in facts.items():
...
def main():
    ...
    parser.add_argument('--output', type=Path,
                        default=Path('lib/loader/Swift/SwiftCDeclarations.inc'))
    ...
    print(f'{len(facts)} fixed Swift C declarations')
