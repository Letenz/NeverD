#!/usr/bin/env python3
"""Extract nonescaping C block parameters, callback ABIs and exact exports."""
import argparse
import ctypes
import json
...
class BlockDeclarations(PrintingClang, CDeclarations):
    ...
    def declaration(self, cursor):
        ...
        for index in range(self.clang_Cursor_getNumArguments(cursor)):
            ...
            if not noescape_attribute(self.pretty(parameter)):
                continue
            signature = self.callback(self.clang_getCursorType(parameter))
            if signature:
                blocks.append([index, signature])
        # Missing or unannotated alternatives veto a lifetime contract.
...
def render(profiles, exports, version, compiler):
    lines = ['// clang-format off',
             ...
             f'// Compiler-derived nonescaping block and export facts: MacOSX SDK {version}.',
    ...
    for name, arm, intel in catalog_rows(profiles):
        ...
        if [arg[0] for arg in a[1]] != [arg[0] for arg in b[1]]:
            continue
...
def main():
    ...
    print(f'verified {sum(line.startswith(chr(123)) for line in output.splitlines())} nonescaping block contracts')
