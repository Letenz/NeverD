...
try:
    ...
    from .generate_objc_framework_declarations import (
        FrameworkDeclarations, module_paths, owns_header, sdk_module_paths,
    )
except ImportError:
    from generate_objc_declarations import CXCursor, CXString, TARGETS, VISITOR
    from generate_objc_framework_declarations import (
...
def main():
    ...
    frameworks = []
    for framework in sorted(set(args.framework or ["Foundation", "CoreData"])):
        ...
        with tempfile.TemporaryDirectory(prefix="neverd-receiver-declarations-") as work:
            ...
            source.write_text(f"#import <{framework}/{framework}.h>\n")
